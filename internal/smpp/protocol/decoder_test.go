...
func TestDecoder_DecodePDU(t *testing.T) {
	tests := []struct {
		name    string
		data    []byte
		wantErr bool
...
func TestDecoder_DecodeBind(t *testing.T) {
	encoder := NewEncoder()
	bind := &BindPDU{
...
func TestDecoder_DecodeSubmitSM(t *testing.T) {
	encoder := NewEncoder()
	submit := &SubmitSMPDU{
		SourceAddr:      "12345",
...
func TestDecoder_ReadCString(t *testing.T) {
	encoder := NewEncoder()
