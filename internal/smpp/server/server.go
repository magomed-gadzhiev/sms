...
// Server представляет SMPP сервер
type Server struct {
...
// NewServer создает новый SMPP сервер
func NewServer(
...
func (s *Server) Start() error {
	...
	s.listener = listener
	s.logger.Info().Str("addr", addr).Msg("SMPP сервер запущен")
...
// Stop останавливает SMPP сервер
func (s *Server) Stop() error {
	s.logger.Info().Msg("остановка SMPP сервера")
	...
	s.logger.Info().Msg("SMPP сервер остановлен")
	return nil
}
		...
		if err != nil {
			if netErr, ok := err.(net.Error); ok && netErr.Timeout() {
				...
				if !session.IsBound() {
					...
					if time.Since(session.LastActivity) > s.config.ReadTimeout*2 {
						s.logger.Warn().
							Str("session_id", session.ID).
		...
		case <-ticker.C:
			...
			for id, session := range s.sessions {
				...
				if now.Sub(session.LastActivity) > 5*time.Minute {
					s.logger.Warn().
						Str("session_id", id).
						Dur("inactive_time", now.Sub(session.LastActivity)).
	...
	for {
		...
		case <-ticker.C:
			...
			for _, session := range s.sessions {
				...
				session.mu.RLock()
				lastSent := session.EnquireLinkSent
				session.mu.RUnlock()
...
func (s *Server) sendEnquireLink(session *Session) {
	...
	if session.Conn != nil {
		...
		session.mu.Lock()
		session.EnquireLinkSent = time.Now()
		session.mu.Unlock()
	}
}
