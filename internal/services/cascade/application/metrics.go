...
var (
	globalCascadeMetrics     *CascadeMetrics
	globalCascadeMetricsOnce sync.Once
)

// NewCascadeMetrics регистрирует и возвращает все метрики cascade-service.
// Metrics are registered once in the default Prometheus registry; subsequent
// calls return the same instance (safe for tests).
func NewCascadeMetrics() *CascadeMetrics {
	globalCascadeMetricsOnce.Do(func() {
		globalCascadeMetrics = &CascadeMetrics{
