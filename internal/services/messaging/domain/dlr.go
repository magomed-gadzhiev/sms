...
// Маппинг stat → статус принадлежит messagestatus; неизвестный stat не даёт
// терминального исхода (по коду ошибки — failed, иначе pending).
func (d *DLRReceipt) GetMessageStatus() string {
	if s, known := messagestatus.FromDLRStat(d.Stat); known {
		return string(s)
	}
	if d.Err != nil && *d.Err != 0 {
		return string(messagestatus.Failed)
	}
	return string(messagestatus.Pending)
}
