...
func TestRetryManager_CalculateNextRetry(t *testing.T) {
	cfg := &config.WorkerConfig{
		RetryBackoffBase: time.Second,
		RetryBackoffMax:  time.Minute,
	}
...
func TestRetryManager_IsPermanentError(t *testing.T) {
	cfg := &config.WorkerConfig{
		RetryBackoffBase: time.Second,
		RetryBackoffMax:  time.Minute,
	}
...
func TestRetryManager_ScheduleRetry(t *testing.T) {
	messageID := uuid.New()
...
func TestRetryManager_MarkAsFailed(t *testing.T) {
	messageID := uuid.New()
	msg := testutil.NewTestMessage()
	msg.ID = messageID
