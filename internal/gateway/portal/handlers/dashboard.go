...
func (h *DashboardHandlers) GetDashboard(w http.ResponseWriter, r *http.Request) {
	...
	// 6b. Получаем топ-5 стран по трафику за 7 дней (FR-002)
	wg.Add(1)
	go func() {
		defer wg.Done()
		if h.analyticsClient == nil {
		...
		if err != nil {
			log.Error().Err(err).Msg("ошибка получения top_countries для дашборда")
			return
		}
		// Sort by total_sent desc, take top 5.
		type cb struct {
			Country string
			Sent    int64
		}
		buckets := make([]cb, 0, len(resp.Groups))
		for _, g := range resp.Groups {
			if g.Stats == nil {
				continue
			}
			buckets = append(buckets, cb{Country: g.Key, Sent: g.Stats.TotalSent})
		}
		// In-place selection-sort top 5 (small N).
		topN := 5
		if topN > len(buckets) {
			topN = len(buckets)
		}
		for i := 0; i < topN; i++ {
			...
			for j := i + 1; j < len(buckets); j++ {
				if buckets[j].Sent > buckets[maxIdx].Sent {
					maxIdx = j
				}
			}
			buckets[i], buckets[maxIdx] = buckets[maxIdx], buckets[i]
		}
		out := make([]map[string]interface{}, 0, topN)
		for i := 0; i < topN; i++ {
			out = append(out, map[string]interface{}{
				"country": buckets[i].Country,
				"count":   buckets[i].Sent,
			})
		}
		topCountries = out
	}()
