...
func TestTryAcquire_UnknownProvider(t *testing.T) {
	m := NewManager()
	assert.True(t, m.TryAcquire(uuid.New()))
}
...
func TestTryAcquire_ExhaustedReturnsFalse(t *testing.T) {
	...
	assert.False(t, m.TryAcquire(id))
}
...
func TestRefill_OverTime(t *testing.T) {
	...
	assert.False(t, m.TryAcquire(id), "should be exhausted")
...
// TestIncrementPending / TestDecrementPending check the in-flight counter.
...
// TestMultipleOperators verifies that each provider has an independent bucket.
func TestMultipleOperators_IndependentBuckets(t *testing.T) {
	...
	// Drain A
	for m.TryAcquire(idA) {
	}
	assert.False(t, m.TryAcquire(idA), "A should be exhausted")

	// B should still have plenty of tokens
	assert.True(t, m.TryAcquire(idB), "B should still have tokens")
...
// TestConcurrentPending verifies that PendingCount is consistent under concurrent
