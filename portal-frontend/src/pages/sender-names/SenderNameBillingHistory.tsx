...
export function SenderNameBillingHistory() {
  usePageTitle('История начислений');
  ...
  const columns: Column<BillingRecord>[] = [
    ...
    },
    {
      key: 'amount',
      header: 'Сумма',
      render: (r) => <span className="font-semibold text-blue-700 dark:text-blue-300">{formatRub(r.amount)}</span>,
    },
    {
      key: 'created_at',
      header: 'Дата начисления',
      render: (r) => <span className="text-sm text-gray-500 dark:text-slate-400">{formatDate(r.created_at)}</span>,
    },
  ];
  ...
  return (
    <div>
      ...
      <div className="mb-3 text-sm text-gray-500 dark:text-slate-400">Всего начислений: {total}</div>
