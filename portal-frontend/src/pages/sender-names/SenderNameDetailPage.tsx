  ...
  if (error || !senderName) {
    return (
      <div>
        ...
        <div className="mt-4 p-3 bg-red-50 dark:bg-red-950/40 text-red-700 dark:text-red-300 rounded-md text-sm">{error || 'Не найдено'}</div>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-4 text-sm mb-6">
        <div>
          <span className="text-gray-400 dark:text-slate-500">Создано</span>
          <div className="text-gray-900 dark:text-slate-100">{formatDate(senderName.created_at)}</div>
        </div>
        {senderName.reviewed_at && (
          <div>
            <span className="text-gray-400 dark:text-slate-500">{senderName.status === 'approved' ? 'Одобрено' : 'Отклонено'}</span>
            <div className="text-gray-900 dark:text-slate-100">{formatDate(senderName.reviewed_at)}</div>
          </div>
        )}
      </div>
      ...
      {senderName.status === 'approved' && (
        <div className="border-t pt-4 mb-6">
          <h3 className="text-sm font-medium text-gray-700 dark:text-slate-300 mb-3">Регистрация у операторов</h3>
              ...
              return (
                <span
                  ...
                  className={`px-3 py-1 rounded-full text-xs font-medium ${
                    registered ? 'bg-green-100 dark:bg-green-900/40 text-green-800 dark:text-green-200' : 'bg-gray-100 dark:bg-slate-800 text-gray-500 dark:text-slate-400'
                  }`}
                >
                  {registered ? '✓ ' : ''}{op.name}
                </span>
        ...
        <div className="border-t pt-4 mb-6">
          {senderName.rejection_reason && (
            <div className="p-3 bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 rounded-md mb-4">
              <p className="text-sm font-medium text-red-800 dark:text-red-300">Причина отклонения</p>
              <p className="text-sm text-red-700 dark:text-red-300 mt-1">{senderName.rejection_reason}</p>
            </div>
          )}
          <h3 className="text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">Исправить и отправить повторно</h3>
          <form onSubmit={handleResubmit} className="flex gap-2 items-start">
            <div>
              ...
              />
              <p className="text-xs text-gray-400 dark:text-slate-500 mt-1">1–11 латинских букв/цифр или 1–15 цифр</p>
              {editError && <p className="text-sm text-red-600 dark:text-red-400 mt-1">{editError}</p>}
            </div>
      ...
      {senderName.status === 'pending' && (
        <div className="border-t pt-4 mb-6">
          <p className="text-sm text-gray-500 dark:text-slate-400">Имя находится на модерации. Мы уведомим вас о результате.</p>
        </div>
      )}
      ...
      <div className="border-t pt-4">
        {!showHistory ? (
          ...
          <div>
            <p className="text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">История статусов</p>
            <div className="space-y-2">
              {history.map((e) => (
                <div key={e.id} className="flex items-start gap-2 text-sm">
                  <span className="text-gray-400 dark:text-slate-500 shrink-0">{formatDate(e.created_at)}</span>
                  <span>
                    {e.old_status ? `${e.old_status} → ` : ''}<strong>{e.new_status}</strong>
                    {e.comment && <span className="text-gray-500 dark:text-slate-400 ml-1">({e.comment})</span>}
                    <span className="text-xs text-gray-400 dark:text-slate-500 ml-1">({e.actor_type})</span>
                  </span>
                </div>
              ))}
              {history.length === 0 && <p className="text-sm text-gray-400 dark:text-slate-500">История пуста</p>}
            </div>
          </div>
        )}
      </div>
