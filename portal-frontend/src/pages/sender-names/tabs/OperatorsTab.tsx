...
export function OperatorsTab({ senderName }: { senderName: SenderNameInfo }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [opsRes, regsRes] = await Promise.all([
        operatorsApi.list(),
        senderNameRegistrationsApi.list(senderName.id),
      ]);
  ...
  const handleResubmit = async (registrationId: string) => {
    ...
    try {
      await senderNameRegistrationsApi.resubmit(senderName.id, registrationId);
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      await senderNameRegistrationsApi.bulkCreate(senderName.id, regs);
