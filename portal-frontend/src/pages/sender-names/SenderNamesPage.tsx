...
export function SenderNamesPage() {
  usePageTitle('Имена отправителей');
  ...
  const columns: Column<SenderNameInfo>[] = [
    ...
    { key: 'next_billing', header: 'Следующее начисление', render: () => <span className="text-xs text-gray-400 dark:text-slate-500">{nextBillingDate()}</span> },
  ...
  return (
    <div>
      ...
      <DataTable
        ...
        bulkActions={[
          {
            label: 'Подать повторно',
            variant: 'primary',
            requiresConfirmation: true,
            confirmMessage: (n) => `Подать повторно ${n} ${n === 1 ? 'отклонённое имя' : 'отклонённых имени'} на модерацию?`,
            onAction: async (_ids, sns) => {
              const rejected = sns.filter((sn) => sn.status === 'rejected');
              await Promise.all(rejected.map((sn) => senderNamesApi.resubmit(sn.id)));
              load();
            },
          },
        ]}
      ...
      <Modal open={showCreate} onClose={() => setShowCreate(false)} title="Зарегистрировать имя отправителя">
        <form onSubmit={handleCreate} className="space-y-4">
          {companies.length > 0 && (
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Компания</label>
              <select
                className="w-full rounded-md border border-gray-300 dark:border-slate-600 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
          ...
          <div>
            <Input
              ...
              maxLength={15}
            />
            <p id="sender-name-hint" className="mt-1 text-xs text-gray-500 dark:text-slate-400">
              1–11 латинских букв/цифр/пробелов или 1–15 цифр
            </p>
            {createError && <p className="mt-1 text-sm text-red-600 dark:text-red-400">{createError}</p>}
          </div>
