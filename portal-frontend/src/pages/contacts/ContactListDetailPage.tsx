...
export function ContactListDetailPage() {
  ...
  usePageTitle(list?.name ?? 'Контактная база');
    ...
    <div>
      ...
      <form onSubmit={handleSearchSubmit} className="mb-4 flex gap-2">
        ...
        <input
          ...
          className="flex-1 max-w-md border border-gray-300 dark:border-slate-600 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
        />
      ...
      <div className="border border-gray-200 dark:border-slate-700 rounded-lg overflow-hidden">
        {loading ? (
          <div className="animate-pulse p-8 text-center text-gray-500 dark:text-slate-400">
            Загрузка...
          </div>
        ) : (
          <>
                ...
                <thead className="bg-gray-50 dark:bg-slate-950 border-b border-gray-200 dark:border-slate-700">
                  <tr>
                    ...
                    {attributes.map((attr) => (
                      <th
                        key={attr.name}
                        className="px-4 py-3 text-left font-medium text-gray-700 dark:text-slate-300 hidden sm:table-cell"
                      >
                  ...
                  {contacts.length === 0 ? (
                    ...
                    contacts.map((contact) => (
                      <tr key={contact.id} className="hover:bg-gray-50 dark:hover:bg-slate-800">
                        <td className="px-4 py-3 font-mono text-gray-800 dark:text-slate-200">
                          {contact.phone}
                        </td>
                        {attributes.map((attr) => (
                          <td
                            key={attr.name}
                            className="px-4 py-3 text-gray-600 dark:text-slate-400 hidden sm:table-cell"
                          >
                        ...
                        <td className="px-4 py-3">
                          <div className="flex gap-1 flex-wrap">
                            {(contact.tags ?? []).map((tag) => (
                              <span
                                key={tag}
                                className="inline-flex items-center rounded-full bg-blue-50 dark:bg-blue-950/40 text-blue-700 dark:text-blue-300 px-2 py-0.5 text-xs"
                              >
                                {tag}
                              </span>
                            ))}
            ...
            {/* Pagination */}
            {total > perPage && (
              <div className="flex items-center justify-between px-4 py-3 border-t border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-950">
      ...
      >
        <div className="space-y-4">
          {addContactError && (
            <div role="alert" className="rounded bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 px-3 py-2 text-sm text-red-700 dark:text-red-300">
              {addContactError}
            </div>
          )}
          <div>
            ...
            <input
              ...
              className="w-full border border-gray-300 dark:border-slate-600 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
      ...
      >
        <div className="space-y-3">
          {editAttrs.map((attr, idx) => (
            <div
              key={idx}
              className="flex gap-2 items-start border border-gray-200 dark:border-slate-700 rounded p-3"
            >
              <div className="flex-1 grid grid-cols-2 gap-2">
                <input
                  ...
                  className="border border-gray-300 dark:border-slate-600 rounded px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
                <input
                  type="text"
