...
export function LookupPage() {
  ...
  const tabTriggerClass =
    'px-4 py-2 text-sm font-medium text-gray-600 dark:text-slate-400 border-b-2 border-transparent data-[state=active]:text-primary data-[state=active]:border-primary hover:text-gray-900 dark:hover:text-slate-100 transition-colors';

  return (
    <div className="max-w-[900px]">
      <PageHeader subtitle="Проверка номеров телефонов" />

      <Tabs.Root defaultValue="single" className="mb-8">
        <Tabs.List className="flex border-b border-gray-200 dark:border-slate-700 mb-6" aria-label="Тип проверки">
        ...
        <Tabs.Content value="single">
          ...
          {singleError && (
            <p role="alert" className="text-red-600 dark:text-red-400 mb-4">{singleError}</p>
          )}

          {singleResult && (
            <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-lg p-5 mb-6">
              <h3 className="text-lg font-semibold mb-3">Результат</h3>
              <dl className="grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
                <div>
                  <dt className="text-gray-500 dark:text-slate-400">Оператор</dt>
                  <dd className="font-medium text-gray-900 dark:text-slate-100">{singleResult.operator_name}</dd>
                </div>
                <div>
                  <dt className="text-gray-500 dark:text-slate-400">Код страны</dt>
                  <dd className="font-medium text-gray-900 dark:text-slate-100">{singleResult.country_code}</dd>
                </div>
                <div>
                  <dt className="text-gray-500 dark:text-slate-400">Статус номера</dt>
                  <dd className="font-medium text-gray-900 dark:text-slate-100">{singleResult.number_status}</dd>
                </div>
                <div>
                  <dt className="text-gray-500 dark:text-slate-400">Тип номера</dt>
                  <dd className="font-medium text-gray-900 dark:text-slate-100">{singleResult.number_type}</dd>
                </div>
                <div>
                  <dt className="text-gray-500 dark:text-slate-400">Портирован</dt>
                  <dd className="font-medium text-gray-900 dark:text-slate-100">{singleResult.is_ported ? 'Да' : 'Нет'}</dd>
                </div>
              </dl>
            </div>
          )}
        ...
        <Tabs.Content value="bulk">
          <form onSubmit={handleBulkLookup} className="mb-6">
            <div
              className={`border-2 border-dashed rounded-lg p-8 text-center cursor-pointer transition-colors mb-4
                ${dragOver ? 'border-primary bg-primary/5' : 'border-gray-300 dark:border-slate-600 hover:border-gray-400 dark:hover:border-slate-500'}`}
            ...
            >
              ...
              {bulkFile ? (
                <div>
                  <p className="text-gray-900 dark:text-slate-100 font-medium">{bulkFile.name}</p>
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500 dark:text-slate-400">Максимум 10 000 номеров</p>
                </div>
              )}
            </div>
          ...
          {bulkError && (
            <p role="alert" className="text-red-600 dark:text-red-400 mb-4">{bulkError}</p>
          )}
