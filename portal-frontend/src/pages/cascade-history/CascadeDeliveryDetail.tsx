  ...
  return (
    <div className="border border-gray-200 rounded-lg p-4">
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
        <div className="text-gray-500">Отправлен:</div>
        <div>{formatDateTime(attempt.sent_at)}</div>
        <div className="text-gray-500">Результат:</div>
        ...
        {attempt.error_message && (
          <>
            <div className="text-gray-500">Ошибка:</div>
            <div className="text-red-600">{attempt.error_message}</div>
          </>
        )}
      </div>
    </div>
    ...
    <div className="p-6 max-w-3xl">
      ...
      <div className="border border-gray-200 rounded-lg p-5 mb-6">
        <div className="grid grid-cols-2 gap-x-8 gap-y-3 text-sm">
          ...
          <div>
            <p className="text-gray-500 text-xs mb-0.5">Доставлено через</p>
            <p className="font-medium">{delivery.delivered_via || '—'}</p>
          </div>
          <div>
            <p className="text-gray-500 text-xs mb-0.5">Получатель</p>
            <p className="font-medium">{delivery.recipient}</p>
          </div>
          <div>
            <p className="text-gray-500 text-xs mb-0.5">Итоговая стоимость</p>
            <p className="font-medium">{delivery.total_cost} {delivery.currency}</p>
          </div>
          <div className="col-span-2">
            <p className="text-gray-500 text-xs mb-0.5">ID</p>
            <p className="font-mono text-xs text-gray-600">{delivery.id}</p>
          </div>
          <div>
            <p className="text-gray-500 text-xs mb-0.5">Создано</p>
            <p>{formatDateTime(delivery.created_at)}</p>
          </div>
          <div>
            <p className="text-gray-500 text-xs mb-0.5">Обновлено</p>
