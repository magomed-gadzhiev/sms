...
export function SubAccountNetworkSection({ subAccountID, subAccountName }: Props) {
  ...
  if (loadError) {
    return (
      <div role="alert" className="bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-300 rounded-lg p-4">
        <p className="font-medium mb-1">Ошибка загрузки</p>
        <p className="text-sm">{loadError}</p>
        <button onClick={loadOverview} className="mt-2 text-sm underline text-red-700 dark:text-red-300 hover:text-red-900">
          Повторить
        </button>
      </div>
    );
  ...
  if (!overview) return <div className="text-sm text-gray-500 dark:text-slate-400 py-6">Загрузка...</div>;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-lg p-4">
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-base font-semibold text-gray-900 dark:text-slate-100 m-0">Provider-set</h3>
          <Button
            variant="secondary"
            onClick={() => {
        ...
        {overview.provider_set ? (
          ...
          <p className="text-sm text-gray-500 dark:text-slate-400 m-0">Provider-set не назначен — суб-аккаунт работает только на override-провайдерах.</p>
        )}
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-lg p-4">
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-base font-semibold text-gray-900 dark:text-slate-100 m-0">Route-set</h3>
          <Button
            variant="secondary"
            onClick={() => {
        ...
        {overview.route_set ? (
          ...
          <p className="text-sm text-gray-500 dark:text-slate-400 m-0">Route-set не назначен.</p>
        )}
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-lg p-4">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-base font-semibold text-gray-900 dark:text-slate-100 m-0">Кастомные провайдеры (override)</h3>
        ...
        {overview.provider_overrides.length === 0 ? (
          <p className="text-sm text-gray-500 dark:text-slate-400 m-0">Override-провайдеров нет.</p>
        ) : (
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {overview.provider_overrides.map((o) => (
                  <tr key={o.provider_id} className="border-b border-gray-100 dark:border-slate-800 last:border-b-0">
                    <td className="py-2 pr-3 font-medium text-gray-900 dark:text-slate-100">{o.name}</td>
                    ...
                    <td className="py-2 pr-3 text-right">
                      <button
                        ...
                        className="text-sm text-red-600 dark:text-red-400 hover:text-red-800 disabled:opacity-50"
                      >
      ...
      <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-lg p-4">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-base font-semibold text-gray-900 dark:text-slate-100 m-0">Кастомные маршруты (overrides)</h3>
          <Button
            onClick={() => {
        ...
        {overview.route_overrides.length === 0 ? (
          <p className="text-sm text-gray-500 dark:text-slate-400 m-0">Override-маршрутов нет.</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              <thead>
              ...
              <tbody>
                {overview.route_overrides.map((o) => (
                  <tr key={o.id} className="border-b border-gray-100 dark:border-slate-800 last:border-b-0">
                    <td className="py-2 pr-3 font-medium text-gray-900 dark:text-slate-100">{o.name || '—'}</td>
