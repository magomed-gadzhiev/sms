  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-16">
        ...
        <span className="ml-3 text-sm text-gray-500">Загрузка дашборда...</span>
      </div>
    );
  }
    ...
    <div className="max-w-6xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
        ...
        <MetricCard title="Трафик">
          <div className="text-2xl font-bold text-gray-900">{formatNumber(data.traffic.total_sent)}</div>
          <div className="mt-2 flex gap-3 text-sm">
            <span className="text-green-600">Доставлено: {formatNumber(data.traffic.total_delivered)}</span>
            <span className="text-red-500">Ошибки: {formatNumber(data.traffic.total_failed)}</span>
          </div>
        </MetricCard>

        <MetricCard title="Delivery Rate">
          <div className={`text-2xl font-bold ${data.traffic.delivery_rate >= 90 ? 'text-green-600' : data.traffic.delivery_rate >= 70 ? 'text-yellow-600' : 'text-red-600'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        ...
        <MetricCard title="Выручка сети">
          <div className="text-sm text-gray-400">{data.revenue.message}</div>
        </MetricCard>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <MetricCard title="Топ-5 субаккаунтов по трафику" className="min-h-[200px]">
          {data.top_sub_accounts.length === 0 ? (
            <div className="text-sm text-gray-400">Нет данных за текущий месяц</div>
          ) : (
              ...
              <tbody>
                {data.top_sub_accounts.map((sa) => (
                  <tr key={sa.id} className="border-t border-gray-100">
                    ...
                    <td className="text-right py-1.5 text-gray-700">{formatNumber(sa.total_sent)}</td>
                    <td className={`text-right py-1.5 ${sa.delivery_rate >= 90 ? 'text-green-600' : sa.delivery_rate >= 70 ? 'text-yellow-600' : 'text-red-600'}`}>
        ...
        <MetricCard title="Проблемные субаккаунты" className="min-h-[200px]">
          {data.problem_sub_accounts.length === 0 ? (
            <div className="text-sm text-green-600">Все субаккаунты в норме</div>
          ) : (
            <ul className="space-y-2">
              {data.problem_sub_accounts.map((p, i) => (
                <li key={`${p.sub_account_id}-${i}`} className="flex items-start gap-2 text-sm">
                  ...
                  <div>
                    <Link to={`/network/sub-accounts/${p.sub_account_id}`} className="font-medium text-gray-900 hover:text-primary">
                      {p.name}
                    </Link>
                    <span className="text-gray-500 ml-1">— {p.detail}</span>
                  </div>
                </li>
              ))}
            </ul>
