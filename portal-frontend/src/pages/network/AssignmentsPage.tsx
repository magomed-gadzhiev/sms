...
export function AssignmentsPage() {
  usePageTitle('Назначения суб-аккаунтам');
  ...
  const statusIcon = (s: NetworkAssignment['validation_status'], err?: string) => {
    if (s === 'ok') return <span className="text-green-600 dark:text-green-400">✓</span>;
    if (s === 'unassigned') return <span className="text-gray-400 dark:text-slate-500">—</span>;
    return (
  ...
  return (
    <div className="max-w-6xl">
      ...
      {/* Sticky bulk-panel */}
      {selected.size > 0 && (
        <div className="sticky top-0 z-10 bg-white dark:bg-slate-900 border border-blue-200 dark:border-blue-800 rounded-lg shadow-sm p-3 mb-4 flex flex-wrap items-center gap-3">
          <span className="text-sm font-medium text-gray-700 dark:text-slate-300">Выбрано: {selected.size}</span>
          <select
            value={bulkSetID}
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.length === 0 ? (
              ...
              filtered.map((a) => (
                <tr key={a.client_id} className="border-t border-gray-100 dark:border-slate-800">
                  <td className="p-3">
                    <input
                      type="checkbox"
                  ...
                  <td className="p-3">
                    <select
                      ...
                      className="rounded border border-gray-300 dark:border-slate-600 px-2 py-1 text-sm min-w-[180px]"
      ...
      >
        <div className="space-y-3">
          ...
          <ul className="text-xs text-gray-600 dark:text-slate-400 max-h-40 overflow-y-auto space-y-1">
            {dryRunResults
