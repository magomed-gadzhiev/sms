...
function TabButton({
  ...
  return (
    ...
    >
      {children}
      {count != null && count > 0 && (
        <span className="ml-1.5 inline-flex items-center justify-center px-1.5 py-0.5 text-xs font-bold rounded-full bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300">
          {count}
        </span>
      )}
    </button>
...
export function ModerationPage() {
  ...
  function StatusFilter({ value, onChange, options }: { value: string; onChange: (v: string) => void; options: { value: string; label: string }[] }) {
    return (
      <select
        ...
        className="border border-gray-300 dark:border-slate-600 rounded px-2 py-1 text-sm"
      >
  ...
  return (
    <div className="max-w-6xl">
      <PageHeader subtitle="Заявки субаккаунтов" />
      ...
      {tab === 'sender_names' && (
        <>
          ...
          {snLoading ? (
            <div className="py-8 text-center text-gray-400 dark:text-slate-500">Загрузка...</div>
          ) : senderNames.length === 0 ? (
            <div className="py-8 text-center text-gray-400 dark:text-slate-500">Нет заявок</div>
          ) : (
