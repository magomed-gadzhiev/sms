  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-6">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-base font-semibold text-gray-800">Текущая квота</h2>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div>
          <div className="text-xs text-gray-500 uppercase mb-1">Лимит сегментов</div>
          <div className="text-2xl font-bold text-gray-900">{formatNumber(quota.segment_limit)}</div>
        </div>
        <div>
          <div className="text-xs text-gray-500 uppercase mb-1">Использовано</div>
          <div className={`text-2xl font-bold ${getProgressTextColor(displayPct)}`}>
            {formatNumber(quota.segments_used)}
          </div>
        </div>
        <div>
          <div className="text-xs text-gray-500 uppercase mb-1">Утилизация</div>
          <div className={`text-2xl font-bold ${getProgressTextColor(displayPct)}`}>
            {displayPct.toFixed(1)}%
          </div>
        </div>
        <div>
          <div className="text-xs text-gray-500 uppercase mb-1">Осталось</div>
...
function HistoryTable({ history }: HistoryTableProps) {
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 mt-6">
      <h3 className="text-sm font-semibold text-gray-700 p-4 border-b border-gray-200">История квот</h3>
                ...
                <tr key={row.id} className="border-t border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="p-3 text-right">
                    {row.overage_segments > 0 ? (
                      <span className="text-red-600 font-medium">{formatNumber(row.overage_segments)}</span>
                    ) : (
...
export function NetworkQuotaPage() {
  ...
  return (
    <div className="max-w-4xl">
      <PageHeader
        title="Квота сегментов"
        actions={
          <button
            onClick={handleLoadHistory}
            disabled={historyLoading}
            className="px-3 py-1.5 text-sm border border-gray-300 rounded-md hover:bg-gray-50 text-gray-700 disabled:opacity-50"
          >
      ...
      {loading ? (
        <div className="py-12 text-center text-gray-400">Загрузка квоты...</div>
      ) : quota === null ? (
        <div className="bg-white rounded-lg border border-gray-200 p-12 text-center">
          <div className="text-gray-400 text-4xl mb-3">📊</div>
          <p className="text-gray-600 font-medium">Активная квота не назначена</p>
          <p className="text-sm text-gray-400 mt-1">Обратитесь к менеджеру для подключения тарифа с квотой сегментов</p>
        </div>
      ) : (
