...
export function AnalyticsPage() {
  ...
  const [range, setRange] = useState<DateRangeValue>({ type: 'preset', preset: '7d' });
  ...
  const loadAnalytics = useCallback(async () => {
    ...
    const useCustomDates = range.type === 'custom';
    const dateFrom = range.type === 'custom' ? range.from : '';
    const dateTo = range.type === 'custom' ? range.to : '';
    const safePeriod: Period =
      range.type === 'preset' && PERIODS.includes(range.preset as Period)
        ? (range.preset as Period)
        : '7d';
    ...
    <div className="max-w-5xl">
      ...
      <fieldset className="border-none p-0 mb-4">
        ...
        <div className="flex gap-2 items-center flex-wrap">
          <DateRangePicker value={range} onChange={setRange} presets={PERIODS} />
          <span className="mx-2 text-gray-600 dark:text-slate-400">|</span>
          <label className="flex items-center gap-1">
            Группировка:
            <select value={groupBy} onChange={(e) => setGroupBy((e.target.value as GroupBy))}
              className="rounded border border-gray-300 dark:border-slate-600 px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary">
      ...
      {data && !loading && (
        <>
          ...
          <Tabs.Root defaultValue="timeline">
            <Tabs.List className="flex gap-1 mb-4 border-b border-gray-200 dark:border-slate-700">
              {['timeline'].map((tab) => (
                <Tabs.Trigger
                  key={tab}
                  value={tab}
                  className="px-4 py-2 text-sm text-gray-600 dark:text-slate-400 hover:text-gray-900 dark:hover:text-slate-100 data-[state=active]:border-b-2 data-[state=active]:border-primary data-[state=active]:text-primary -mb-px"
                >
                  Хронология
                </Tabs.Trigger>
              ))}
