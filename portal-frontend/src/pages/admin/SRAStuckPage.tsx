...
export function SRAStuckPage() {
  ...
  return (
    <div className="p-6">
      <h1 className="text-xl font-semibold text-gray-900 mb-1">Залипшие SRA-rows (cap=100)</h1>
      ...
      {!loading && !error && rows !== null && rows.length === 0 && (
        <p className="text-gray-500 text-sm">Нет залипших row.</p>
      )}
              ...
              {rows.map((row) => (
                <tr key={row.client_id} className="hover:bg-gray-50">
                  <td className="px-4 py-3">
                    <div className="font-medium text-gray-900">{row.client_email}</div>
                    <div className="text-xs text-gray-400">{row.client_id}</div>
                  </td>
                  <td className="px-4 py-3 text-gray-700">{row.provider_set_id ?? '—'}</td>
                  <td className="px-4 py-3 text-gray-700">{row.route_set_id ?? '—'}</td>
