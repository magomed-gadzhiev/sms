      ...
      {template && (
        <div className="space-y-5">
          ...
          <div className="flex gap-4 text-sm text-gray-600 dark:text-slate-400">
            <div>
              <span className="font-medium text-gray-900 dark:text-slate-100">Шаблон:</span> {template.name}
            </div>
            <div>
              <span className="font-medium text-gray-900 dark:text-slate-100">Клиент:</span> {template.client_id.slice(0, 8)}...
            </div>
          </div>

          {/* Template body */}
          <div className="rounded border border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-950 p-4">
            <div className="text-xs font-medium text-gray-500 dark:text-slate-400 mb-2 uppercase tracking-wide">Текст шаблона</div>
            <pre className="whitespace-pre-wrap font-mono text-sm text-gray-800 dark:text-slate-200 leading-relaxed">
          ...
          <div className="space-y-3">
            ...
            {revisionMode && (
              <div className="space-y-3 rounded border border-yellow-200 dark:border-yellow-800 bg-yellow-50 dark:bg-yellow-950/40 p-3">
                <label className="block text-sm font-medium text-gray-700 dark:text-slate-300">
                  Комментарий для доработки <span className="text-danger">*</span>
                </label>
                <textarea
                  className="w-full rounded border border-gray-300 dark:border-slate-600 px-3 py-2 text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-primary/50"
                  rows={3}
                  value={comment}
            ...
            {rejectMode && (
              <div className="space-y-3 rounded border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-950/40 p-3">
                <label className="block text-sm font-medium text-gray-700 dark:text-slate-300">
          ...
          {/* Audit timeline */}
          <div>
            <div className="text-sm font-semibold text-gray-900 dark:text-slate-100 mb-3">История</div>
            {loading ? (
              <div className="text-sm text-gray-500 dark:text-slate-400">Загрузка...</div>
            ) : auditEntries.length === 0 ? (
              <div className="text-sm text-gray-500 dark:text-slate-400">Нет записей</div>
            ) : (
              <div>
