      ...
      <div className="space-y-6 max-w-2xl">
        ...
        {SECTIONS.map((section) => {
          ...
          return (
            <div
              key={section.title}
              className={`bg-white dark:bg-slate-900 border rounded-lg p-6 transition-colors ${
                changed ? 'border-yellow-400 shadow-sm' : 'border-gray-200 dark:border-slate-700'
              }`}
            >
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-base font-semibold text-gray-900 dark:text-slate-100">{section.title}</h2>
                {changed && (
                  <span className="text-xs text-yellow-600 dark:text-yellow-400 bg-yellow-50 dark:bg-yellow-950/40 border border-yellow-200 dark:border-yellow-800 px-2 py-0.5 rounded-full">
              ...
              <div className="space-y-4">
                {section.fields.map((field) => (
                  <div key={field.key}>
                    ...
                    />
                    <p className="mt-1 text-xs text-gray-500 dark:text-slate-400">{field.description}</p>
                  </div>
                ))}
              </div>

              {changed && (
                <div className="mt-4 flex justify-end gap-3 pt-4 border-t border-gray-100 dark:border-slate-800">
