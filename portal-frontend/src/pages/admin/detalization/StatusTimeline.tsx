...
export const StatusTimeline: React.FC<StatusTimelineProps> = ({ statuses }) => {
  if (!statuses || statuses.length === 0) {
    return <p className="text-sm text-gray-500 dark:text-slate-400">История статусов недоступна</p>;
  }
        ...
        return (
          <div key={`${event.status}-${event.timestamp}-${idx}`} className="flex gap-3">
            ...
            <div className="flex flex-col items-center">
              ...
              {!isLast && <div className="w-px flex-1 min-h-8 bg-gray-200 dark:bg-slate-700 mt-1" />}
            </div>
            {/* Content */}
            <div className="pb-4">
              <p className="text-sm font-medium text-gray-800 dark:text-slate-200">{label}</p>
              <p className="text-xs text-gray-500 dark:text-slate-400">{formatTime(event.timestamp)}</p>
              {event.details && (
                <p className="text-xs text-gray-600 dark:text-slate-400 mt-0.5">{event.details}</p>
              )}
            </div>
          </div>
        );
