...
export function SenderNamesAdminPage() {
  ...
  const columns: Column<AdminSenderNameInfo>[] = [
    ...
    { key: 'client_id', header: 'Клиент', render: (sn) => <span className="text-xs text-gray-500 dark:text-slate-400">{sn.client_id}</span> },
    {
    ...
    <div>
      ...
      <div className="flex gap-3 mb-4">
        <select
          ...
          className="border border-gray-300 dark:border-slate-600 rounded-md px-3 py-1.5 text-sm"
        >
        ...
        <input
          ...
          className="border border-gray-300 dark:border-slate-600 rounded-md px-3 py-1.5 text-sm w-48"
        />
      </div>
      ...
      <Modal open={showAddModal} onClose={() => { setShowAddModal(false); setNameError(''); }} title="Добавить имя отправителя">
        <form onSubmit={handleAddSubmit} className="space-y-4">
          ...
          <div className="mt-1 space-y-0.5">
            <p className="text-xs text-gray-500 dark:text-slate-400">Имя должно совпадать с названием организации, ИП, товарным знаком или доменом</p>
            <p className="text-xs text-gray-500 dark:text-slate-400">Латиница, не более 11 символов. Можно использовать цифры и знаки . _ —</p>
            <p className="text-xs text-gray-500 dark:text-slate-400">Пробелы запрещены</p>
      ...
      <Modal open={!!rejectTarget} onClose={() => setRejectTarget(null)} title="Отклонить имя отправителя">
        <form onSubmit={handleReject} className="space-y-4">
          <p className="text-sm text-gray-600 dark:text-slate-400">Имя: <strong className="font-mono">{rejectTarget?.name}</strong></p>
          <Input
      ...
      <Modal open={!!deactivateTarget} onClose={() => setDeactivateTarget(null)} title="Деактивировать имя отправителя">
        <form onSubmit={handleDeactivate} className="space-y-4">
          <p className="text-sm text-gray-600 dark:text-slate-400">Имя: <strong className="font-mono">{deactivateTarget?.name}</strong></p>
          <Input
            ...
            placeholder="Укажите причину..."
          />
          {deactivateError && <p className="text-sm text-red-600 dark:text-red-400">{deactivateError}</p>}
          <div className="flex justify-end gap-2">
