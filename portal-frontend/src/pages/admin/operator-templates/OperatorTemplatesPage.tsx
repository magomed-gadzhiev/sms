...
export function OperatorTemplatesPage() {
  ...
  const columns: Column<OperatorTemplate>[] = [
    ...
    },
    {
      ...
      header: 'Текст',
      render: (row) => (
        <span className="text-xs text-gray-600 dark:text-slate-400 truncate max-w-[250px] block" title={row.body}>
          {row.body}
        </span>
      ),
    },
      ...
      render: (row) =>
        row.variables && row.variables.length > 0 ? (
          <div className="flex flex-wrap gap-1">
            {row.variables.map((v) => (
              <span key={v} className="px-1.5 py-0.5 bg-gray-100 dark:bg-slate-800 rounded text-xs font-mono">{v}</span>
            ))}
          </div>
        ) : (
    ...
    <>
      ...
      >
        <div className="space-y-4">
          ...
          />
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Текст шаблона *</label>
            <div className="mb-2 flex flex-wrap gap-1">
              <span className="text-xs text-gray-500 dark:text-slate-400 mr-1">Вставить переменную:</span>
              {VARIABLE_HINTS.map((v) => (
                <button
                  key={v}
                  type="button"
                  onClick={() => insertVariable(v)}
                  className="px-2 py-0.5 text-xs bg-blue-50 dark:bg-blue-950/40 hover:bg-blue-100 text-blue-700 dark:text-blue-300 rounded border border-blue-200 dark:border-blue-800 font-mono transition-colors"
                >
                  {v}
                </button>
              ))}
            ...
            <textarea
              className="w-full border border-gray-300 dark:border-slate-600 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 font-mono"
              rows={5}
              value={form.body}
            ...
            />
            {form.body && (
              <div className="mt-2 p-3 bg-gray-50 dark:bg-slate-950 rounded-lg border text-sm">
                <p className="text-xs text-gray-500 dark:text-slate-400 mb-1 font-medium">Предпросмотр с выделенными переменными:</p>
                <p className="leading-relaxed break-words">
                  {form.body.split(/(\{[^}]+\})/g).map((part, i) =>
                    /^\{[^}]+\}$/.test(part) ? (
                      <span key={i} className="text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/40 px-1 rounded font-medium">{part}</span>
                    ) : (
            ...
            {form.body && (
              <div className="mt-2">
                <p className="text-xs text-gray-500 dark:text-slate-400 mb-1 font-medium">Предпросмотр с примером значений:</p>
                <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-xl p-4 shadow-sm max-w-xs">
                  <p className="text-xs text-gray-400 dark:text-slate-500 font-medium mb-1">
