...
export function AnalyticsPage() {
  ...
  return (
    <>
      ...
      <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-lg mb-6">
        <div className="flex items-center gap-1 px-4 pt-4 pb-0 border-b border-gray-200 dark:border-slate-700">
          {GROUP_TABS.map((tab) => (
            <button
              key={tab.value}
        ...
        <div className="h-[520px] overflow-y-auto relative border-t border-gray-200 dark:border-slate-700">
          <table className="w-full text-sm">
            <thead className="sticky top-0 bg-gray-50 dark:bg-slate-950 z-10 shadow-sm">
              <tr className="border-b border-gray-200 dark:border-slate-700">
                <th className="text-left px-4 py-2 font-medium text-gray-600 dark:text-slate-400">{labelHeader}</th>
                <th className="text-right px-4 py-2 font-medium text-gray-600 dark:text-slate-400">Отправлено</th>
                <th className="text-right px-4 py-2 font-medium text-gray-600 dark:text-slate-400">Доставлено</th>
                <th className="text-right px-4 py-2 font-medium text-gray-600 dark:text-slate-400">Ошибки</th>
                <th className="text-right px-4 py-2 font-medium text-gray-600 dark:text-slate-400">Доставляемость</th>
                <th className="text-right px-4 py-2 font-medium text-gray-600 dark:text-slate-400">Стоимость</th>
              </tr>
            </thead>
            <tbody>
              {loading && (
                <tr>
                  <td colSpan={6} className="text-center py-12 text-gray-400 dark:text-slate-500">Загрузка...</td>
                </tr>
              )}
              {!loading && rows.length === 0 && (
                <tr>
                  <td colSpan={6} className="text-center py-12 text-gray-400 dark:text-slate-500">Нет данных за выбранный период</td>
                </tr>
              )}
              {!loading && rows.map((row) => (
                <tr key={row.label} className="border-b border-gray-100 dark:border-slate-800 hover:bg-gray-50 dark:hover:bg-slate-800">
                  <td className="px-4 py-2 text-gray-900 dark:text-slate-100">{row.label}</td>
                  <td className="px-4 py-2 text-right text-gray-700 dark:text-slate-300">{fmtNum(row.sent)}</td>
                  <td className="px-4 py-2 text-right text-green-700 dark:text-green-300">{fmtNum(row.delivered)}</td>
                  <td className="px-4 py-2 text-right text-red-600 dark:text-red-400">{fmtNum(row.failed)}</td>
                  <td className="px-4 py-2 text-right text-gray-700 dark:text-slate-300">{fmtRate(row.delivery_rate)}</td>
                  <td className="px-4 py-2 text-right text-gray-700 dark:text-slate-300">{fmtCost(row.cost)}</td>
                </tr>
              ))}
            </tbody>
            {!loading && totals && rows.length > 0 && (
              <tfoot className="sticky bottom-0 bg-gray-50 dark:bg-slate-950 font-semibold border-t-2 border-gray-200 dark:border-slate-700">
                <tr className="text-gray-900 dark:text-slate-100">
                  ...
                  <td className="px-4 py-2 text-right text-green-700 dark:text-green-300">{fmtNum(totals.delivered)}</td>
                  <td className="px-4 py-2 text-right text-red-600 dark:text-red-400">{fmtNum(totals.failed)}</td>
      ...
      {!loading && stats?.timeline && stats.timeline.length > 0 && (
        <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-lg p-4 mb-6">
          <h3 className="text-sm font-medium text-gray-600 dark:text-slate-400 mb-3">Объём сообщений</h3>
      ...
      {!loading && stats?.by_provider && stats.by_provider.length > 0 && (
        <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-lg p-4">
          <h3 className="text-sm font-medium text-gray-600 dark:text-slate-400 mb-3">По провайдерам</h3>
