...
export function LegalEntitiesPage() {
  ...
  const columns: Column<LegalEntity>[] = [
    ...
    { key: 'name', header: 'Краткое название' },
    {
      ...
      render: (row) => <span className="text-sm text-gray-600 dark:text-slate-400">{row.full_name || '—'}</span>,
    },
    {
      key: 'address',
      header: 'Адрес',
      render: (row) => <span className="text-sm text-gray-600 dark:text-slate-400">{row.address || '—'}</span>,
    },
    {
      key: 'operators',
      header: 'Операторы',
      render: (row) => {
        ...
        return <span className="text-sm text-gray-600 dark:text-slate-400">{operators?.join(', ') ?? '—'}</span>;
      },
    },
    {
      key: 'active',
      ...
      >
        <div className="space-y-4">
          <Input
            ...
            maxLength={12}
          />
          {innError && <p className="text-xs text-red-500 dark:text-red-400 mt-1">{innError}</p>}
          <Input
