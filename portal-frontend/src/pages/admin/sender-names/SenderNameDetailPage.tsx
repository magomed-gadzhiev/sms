...
export function SenderNameDetailPage() {
  ...
  if (loading) {
    return <div className="text-center py-12 text-gray-400 dark:text-slate-500">Загрузка...</div>;
  }

  if (error || !senderName) {
    return (
      <div className="p-6">
        <div className="p-3 bg-red-50 dark:bg-red-950/40 text-red-700 dark:text-red-300 rounded-md text-sm mb-4">{error || 'Не найдено'}</div>
  ...
  return (
    <>
      ...
      <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-lg p-6 mb-6">
        <h2 className="text-base font-semibold text-gray-900 dark:text-slate-100 mb-4">Общая информация</h2>
        <dl className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div>
            <dt className="text-xs text-gray-500 dark:text-slate-400 mb-1">Имя</dt>
            <dd className="font-mono font-medium text-gray-900 dark:text-slate-100">{senderName.name}</dd>
          </div>
          <div>
            <dt className="text-xs text-gray-500 dark:text-slate-400 mb-1">Клиент</dt>
            <dd className="text-sm text-gray-700 dark:text-slate-300">{senderName.client_id}</dd>
          </div>
          <div>
            <dt className="text-xs text-gray-500 dark:text-slate-400 mb-1">Статус</dt>
            <dd><Badge variant={statusInfo.variant}>{statusInfo.label}</Badge></dd>
          </div>
          <div>
            <dt className="text-xs text-gray-500 dark:text-slate-400 mb-1">Создано</dt>
            <dd className="text-sm text-gray-700 dark:text-slate-300">{formatDate(senderName.created_at)}</dd>
          </div>
          {senderName.rejection_reason && (
            <div className="col-span-2 md:col-span-4">
              <dt className="text-xs text-gray-500 dark:text-slate-400 mb-1">Причина отклонения</dt>
              <dd className="text-sm text-red-700 dark:text-red-300">{senderName.rejection_reason}</dd>
            </div>
          )}
        </dl>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-lg p-6">
        <h2 className="text-base font-semibold text-gray-900 dark:text-slate-100 mb-4">Регистрация у операторов</h2>
        {registrations.length === 0 ? (
          <p className="text-sm text-gray-400 dark:text-slate-500">Данные о регистрации у операторов отсутствуют</p>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            {registrations.map((reg) => {
              const regStatus = REG_STATUS_BADGE[reg.status] ?? { variant: 'default' as const, label: reg.status };
              return (
                <div key={reg.operator_id} className="border border-gray-200 dark:border-slate-700 rounded-lg p-3">
                  <div className="text-sm font-medium text-gray-900 dark:text-slate-100 mb-1 truncate">{reg.operator_name}</div>
                  <div className="text-xs text-gray-400 dark:text-slate-500 mb-2">MCC {reg.mcc} / MNC {reg.mnc}</div>
                  <Badge variant={regStatus.variant}>{regStatus.label}</Badge>
                  {reg.registered_at && (
                    <div className="text-xs text-gray-400 dark:text-slate-500 mt-1">{formatDate(reg.registered_at)}</div>
                  )}
                </div>
              );
            })}
      ...
      <Modal open={showReject} onClose={() => setShowReject(false)} title="Отклонить имя отправителя">
        <form onSubmit={handleReject} className="space-y-4">
          <p className="text-sm text-gray-600 dark:text-slate-400">Имя: <strong className="font-mono">{senderName.name}</strong></p>
          <Input
