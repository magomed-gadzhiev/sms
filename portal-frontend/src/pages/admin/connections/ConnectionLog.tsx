...
export const ConnectionLog: React.FC<ConnectionLogProps> = ({ connectionId }) => {
  ...
  if (!LOG_ENDPOINT_EXISTS) {
    return (
      <div className="bg-gray-900 rounded-lg p-4 text-gray-400 dark:text-slate-500 text-sm font-mono h-48 flex items-center justify-center">
  ...
  return (
    <div className="bg-gray-900 rounded-lg p-3 h-48 overflow-y-auto font-mono text-xs">
      {lines.length === 0 ? (
        <span className="text-gray-500 dark:text-slate-400">Нет логов</span>
      ) : (
        lines.map((line, i) => (
          <div key={`${line.timestamp}-${i}`} className="leading-5">
            <span className="text-gray-500 dark:text-slate-400">[{line.timestamp}]</span>{' '}
            <span className={
