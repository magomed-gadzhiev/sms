...
export function WebhooksPage() {
  ...
  return (
    <>
      ...
      {!selectedClientID ? (
        <div className="text-center py-12 text-gray-500 dark:text-slate-400 border border-dashed rounded">
      ...
      <Modal open={showCreate} onClose={() => setShowCreate(false)} title="Создание вебхука">
        <div className="space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 dark:text-slate-400 mt-1">Доступные: {VALID_EVENT_TYPES.join(', ')}</p>
            {invalidEvents.length > 0 && (
              <p className="text-xs text-red-600 dark:text-red-400 mt-1">Недопустимые: {invalidEvents.join(', ')}</p>
            )}
          </div>
