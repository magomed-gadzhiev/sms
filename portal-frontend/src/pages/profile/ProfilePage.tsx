  ...
  return (
    <div className="max-w-xl">
      ...
      <section className="mb-8">
        <p className="text-gray-700 dark:text-slate-300"><strong>Email:</strong> {profile.email}</p>
        <p className="text-gray-700 dark:text-slate-300"><strong>Компания:</strong> {profile.company_name}</p>

        <form onSubmit={handleSaveProfile} className="mt-4">
          {saveMsg && saveIsError && (
            <p id="profile-error" role="alert" className="text-red-600 dark:text-red-400 mb-3">{saveMsg}</p>
          )}
          <div className="mb-2">
            <Input
          ...
          <div className="flex items-center gap-3">
            ...
            {saveMsg && !saveIsError && (
              <p role="status" className="inline text-green-600 dark:text-green-400">{saveMsg}</p>
            )}
          </div>
        </form>
      </section>

      <section className="border border-gray-200 dark:border-slate-700 rounded-lg p-6">
        <h3 className="text-lg font-semibold mb-4">Двухфакторная аутентификация</h3>
        {twoFaMsg && twoFaIsError && (
          <p id="profile-2fa-error" role="alert" className="text-red-600 dark:text-red-400 mb-3">{twoFaMsg}</p>
        )}
        {twoFaMsg && !twoFaIsError && (
          <p role="status" className="text-green-600 dark:text-green-400 mb-3">{twoFaMsg}</p>
        )}
        ...
        {profile.totp_enabled ? (
          ...
          <div>
            ...
            <div className="bg-gray-50 dark:bg-slate-950 rounded-lg p-4 mb-4 inline-block">
              <img src={totpSetup.qr_code_url} alt="TOTP QR Code" className="max-w-[200px]" />
            </div>
            <p className="mb-3">
              Или введите секрет вручную:{' '}
              <code className="bg-white dark:bg-slate-900 px-2 py-1 rounded border text-sm">{totpSetup.secret}</code>
              {' '}
              <Button
                type="button"
                variant="secondary"
      ...
      <section className="border border-gray-200 dark:border-slate-700 rounded-lg p-6 mt-8">
        <h3 className="text-lg font-semibold mb-4">Сменить пароль</h3>

        {pwMsg && pwIsError && (
          <p id="pw-error" role="alert" className="text-red-600 dark:text-red-400 mb-3">{pwMsg}</p>
        )}
        {pwMsg && !pwIsError && (
          <p role="status" className="text-green-600 dark:text-green-400 mb-3">{pwMsg}</p>
        )}
        ...
        <form onSubmit={handleChangePassword}>
          ...
          <div className="mb-3">
            <Input
              ...
              aria-describedby={pwMsg && pwIsError ? 'pw-error' : undefined}
            />
            <p className="text-xs text-gray-500 dark:text-slate-400 mt-1">Минимум 8 символов</p>
          </div>
          <div className="mb-4">
            <Input
