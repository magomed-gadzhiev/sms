...
const columns: Column<TransactionItem>[] = [
  ...
  },
  {
    ...
    render: (tx) => {
      ...
      return (
        <span className={isOutflow ? 'text-red-600 dark:text-red-400 font-medium' : 'text-green-600 dark:text-green-400 font-medium'}>
...
export function BillingPage() {
  ...
  return (
    <div>
      {isReseller && (
        <div className="mb-4 bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800 rounded-lg px-4 py-3 text-sm text-blue-800 dark:text-blue-300">
          Отображается баланс и транзакции вашего аккаунта. Балансы суб-аккаунтов и переводы доступны
          в разделе <Link to="/sub-accounts" className="font-medium underline hover:text-blue-900 dark:hover:text-blue-200">Суб-аккаунты</Link>.
        </div>
      )}
      ...
      {companies.length > 0 && (
        <div className="mb-6">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {companies.map((c) => (
              <div key={c.id} className="bg-white dark:bg-slate-900 rounded-lg border border-gray-200 dark:border-slate-700 p-4">
                <div className="flex items-start justify-between">
                  <div>
                    <p className="text-sm font-medium text-gray-900 dark:text-slate-100">{c.name}</p>
                    {c.inn && <p className="text-xs text-gray-500 dark:text-slate-400 mt-0.5">ИНН: {c.inn}</p>}
                  </div>
      ...
      {/* Balance card */}
      <div className="mb-6 rounded-lg border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-6 shadow-sm">
        {balanceError && (
          <p role="alert" className="text-red-600 dark:text-red-400 text-sm mb-2">{balanceError}</p>
        )}
        {balanceLoading && !balance ? (
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-500 dark:text-slate-400 mb-1">Текущий баланс</p>
      ...
      <div className="mb-6 rounded-lg border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-6 shadow-sm">
        ...
        {thresholdError && (
          <p role="alert" className="text-red-600 dark:text-red-400 text-sm mt-2">{thresholdError}</p>
        )}
        {thresholdSaved && (
          <p role="status" className="text-green-600 dark:text-green-400 text-sm mt-2">Порог сохранён</p>
        )}
      </div>
      ...
      >
        <div className="space-y-4">
          {topUpError && (
            <p role="alert" className="text-red-600 dark:text-red-400 text-sm">{topUpError}</p>
          )}
          <Input
            label="Сумма (₽)"
            type="number"
            ...
            step="0.01"
          />
          <p className="text-xs text-gray-400 dark:text-slate-500">Минимум: 1 ₽ · Максимум: 1 000 000 ₽</p>
      ...
      {error && <div role="alert" className="text-red-600 dark:text-red-400 mb-3">Ошибка: {error}</div>}

      {!loading && !error && (data?.transactions?.length ?? 0) === 0 && (
        <div className="text-center py-10 text-gray-500 dark:text-slate-400">
          <svg className="mx-auto mb-3 w-10 h-10 text-gray-300 dark:text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
          ...
          <p className="text-xs text-gray-400 dark:text-slate-500 mt-1">Здесь будут отображаться пополнения, списания и возвраты</p>
        </div>
      )}
