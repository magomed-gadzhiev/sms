...
export function CompanyDetailPage() {
  ...
  usePageTitle(company?.name ?? 'Компания');
  ...
  if (loading) return <div className="p-8 text-gray-500 dark:text-slate-400">Загрузка...</div>;
  if (!company) return <div className="p-8 text-red-600 dark:text-red-400">{error || 'Компания не найдена'}</div>;
  ...
  return (
    <div>
      ...
      {success && <div className="mb-4 p-3 bg-green-50 dark:bg-green-950/40 text-green-700 dark:text-green-300 rounded-md text-sm">{success}</div>}

      {isOffer ? (
        <div className="max-w-2xl">
          <div className="bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800 rounded-lg p-4 mb-6 text-sm text-blue-800 dark:text-blue-300">
        ...
        <form onSubmit={handleSave} className="space-y-6 max-w-2xl">
          {/* Основные реквизиты */}
          <section className="bg-white dark:bg-slate-900 rounded-lg border border-gray-200 dark:border-slate-700 p-6 space-y-4">
            <h3 className="text-sm font-semibold text-gray-700 dark:text-slate-300 uppercase tracking-wide">Основные реквизиты</h3>
          ...
          {/* Руководитель */}
          <section className="bg-white dark:bg-slate-900 rounded-lg border border-gray-200 dark:border-slate-700 p-6 space-y-4">
            <h3 className="text-sm font-semibold text-gray-700 dark:text-slate-300 uppercase tracking-wide">Руководитель</h3>
          ...
          {/* Адреса */}
          <section className="bg-white dark:bg-slate-900 rounded-lg border border-gray-200 dark:border-slate-700 p-6 space-y-4">
            <h3 className="text-sm font-semibold text-gray-700 dark:text-slate-300 uppercase tracking-wide">Адреса</h3>
          ...
          <section className="bg-white dark:bg-slate-900 rounded-lg border border-gray-200 dark:border-slate-700 p-6 space-y-4">
            <h3 className="text-sm font-semibold text-gray-700 dark:text-slate-300 uppercase tracking-wide">Банковские реквизиты</h3>
          ...
          {/* Контакты */}
          <section className="bg-white dark:bg-slate-900 rounded-lg border border-gray-200 dark:border-slate-700 p-6 space-y-4">
            <h3 className="text-sm font-semibold text-gray-700 dark:text-slate-300 uppercase tracking-wide">Контакты</h3>
            <div className="grid grid-cols-2 gap-4">
