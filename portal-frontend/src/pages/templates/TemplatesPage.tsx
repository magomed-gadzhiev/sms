...
export function TemplatesPage() {
  ...
  const columns: Column<TemplateInfo>[] = [
    ...
    },
    {
      ...
      header: 'Текст',
      render: (tpl) => (
        <span className="text-gray-600 dark:text-slate-400 max-w-[250px] inline-block truncate" title={tpl.body}>
    ...
    <div className="w-full">
        ...
        <form onSubmit={handleSubmit}>
          <div className="mb-4">
            ...
            {tplValidation.errors.name && (
              <p id="name-error" className="mt-1 text-xs text-red-600 dark:text-red-400">{tplValidation.errors.name}</p>
            )}
          </div>
          ...
          <div className="mb-4">
            ...
            {sendersError ? (
              ...
              <select
                ...
                className="w-full rounded border border-gray-300 dark:border-slate-600 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary"
              >
          ...
          <div className="mb-4">
            <div className="flex items-center justify-between mb-1">
              <label htmlFor="tpl-body" className="block text-sm font-medium text-gray-700 dark:text-slate-300">
                Текст шаблона *
              </label>
            ...
            <textarea
              ...
              className={`w-full rounded border px-3 py-2 text-sm transition-colors focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary ${tplValidation.errors.body ? 'border-red-400' : 'border-gray-300 dark:border-slate-600'}`}
            />
            <div className="mt-2 flex flex-wrap gap-1.5" role="toolbar" aria-label="Вставить переменную">
              {[
                ...
                <button
                  ...
                  className="px-2.5 py-1 text-xs rounded-full border border-gray-300 dark:border-slate-600 bg-gray-50 dark:bg-slate-950 hover:bg-gray-100 dark:hover:bg-slate-700 hover:border-gray-400 dark:hover:border-slate-500 text-gray-700 dark:text-slate-300 transition-colors"
                >
                  {chip.label}
                </button>
              ))}
            ...
            {tplValidation.errors.body ? (
              <p id="body-error" className="mt-1 text-xs text-red-600 dark:text-red-400">{tplValidation.errors.body}</p>
            ) : (
              <p id="body-hint" className="mt-1 text-xs text-gray-500 dark:text-slate-400">
      ...
      >
        <div className="space-y-4">
          <div className="bg-gray-50 dark:bg-slate-950 rounded p-3 text-sm">
            <p className="text-gray-500 dark:text-slate-400 text-xs mb-1">Исходный текст:</p>
          ...
          {Object.keys(previewVars).length > 0 ? (
            ...
            <p className="text-sm text-gray-500 dark:text-slate-400">Шаблон не содержит переменных.</p>
          )}
          ...
          {previewResult !== null && (
            <div className="bg-green-50 dark:bg-green-950/40 border border-green-200 dark:border-green-800 rounded p-3">
              <p className="text-xs text-green-700 dark:text-green-300 mb-1">Результат:</p>
        ...
        rowActions={(tpl) => (
          <div className="flex gap-1 flex-wrap">
            ...
            {tpl.status === 'rejected' && tpl.rejection_reason && (
              <span
                className="inline-flex items-center text-xs text-red-600 dark:text-red-400 ml-1"
