...
function StatusBadge({ status }: { status: string }) {
  ...
  const colorMap: Record<string, string> = {
    delivered: 'bg-green-100 dark:bg-green-900/40 text-green-800 dark:text-green-200',
    ...
    expired: 'bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-slate-400',
  ...
  const cls = colorMap[status] ?? 'bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-slate-300';
  return (
...
export function QuickSendPage() {
  ...
  return (
    <div>
      ...
      <div className="max-w-2xl bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-lg p-6 mb-8">
        <div className="space-y-5">
          {/* Текст сообщения */}
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1" htmlFor="qs-text">
              Текст сообщения
            </label>
            <textarea
              id="qs-text"
              className="w-full border border-gray-300 dark:border-slate-600 rounded-md px-3 py-2 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary"
              rows={4}
          ...
          {/* Контакты */}
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1" htmlFor="qs-contacts">
              Контакты
            </label>
            <p className="text-xs text-gray-500 dark:text-slate-400 mb-1">Вставьте номера списком — по одному на строку (или через запятую)</p>
            <textarea
              id="qs-contacts"
              className="w-full border border-gray-300 dark:border-slate-600 rounded-md px-3 py-2 text-sm resize-none font-mono focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary"
              rows={5}
          ...
          <div>
            {loadingSenders ? (
              <>
                <span className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Имя отправителя</span>
                <div className="h-9 bg-gray-100 dark:bg-slate-800 rounded animate-pulse" />
              </>
            ) : sendersError ? (
              <>
                <span className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Имя отправителя</span>
      ...
      {sentMessages.length > 0 && (
        <div ref={tableRef}>
          <div className="flex items-center gap-3 mb-3">
            ...
            {polling && (
              <span className="flex items-center gap-1 text-xs text-green-600 dark:text-green-400" title="Статусы обновляются в реальном времени">
          ...
          <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-lg overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 dark:bg-slate-950 border-b border-gray-200 dark:border-slate-700">
                <tr>
                  <th className="text-left px-4 py-3 font-medium text-gray-600 dark:text-slate-400">Получатель</th>
                  <th className="text-left px-4 py-3 font-medium text-gray-600 dark:text-slate-400">Текст</th>
                  <th className="text-left px-4 py-3 font-medium text-gray-600 dark:text-slate-400">Статус</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100 dark:divide-slate-800">
                {sentMessages.map((msg) => (
                    <tr key={msg.message_id} className="hover:bg-gray-50 dark:hover:bg-slate-800">
