...
export function TariffsPage() {
  ...
  return (
    <div>
      <PageHeader subtitle="Действующие цены по операторам" />
      ...
      {loading && !data ? (
        <div className="animate-pulse p-8 text-center text-gray-500 dark:text-slate-400">Загрузка...</div>
      ) : error ? (
  ...
  return (
    <label className="flex items-center gap-2 text-sm">
      ...
      <select
        ...
        className="rounded border border-slate-300 px-2 py-1 text-sm bg-white dark:bg-slate-900"
      >
        {options.map((o) => (
