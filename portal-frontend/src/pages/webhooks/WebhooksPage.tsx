  ...
  return (
    <div className="max-w-[900px]">
      ...
      {createdSecret && (
        <div
          role="alert"
          className="bg-green-50 dark:bg-green-950/40 border border-green-500 rounded p-4 mb-4"
        >
          ...
          <button
            ...
            className="mt-2 bg-transparent border-none cursor-pointer underline text-sm text-gray-600 dark:text-slate-400"
          >
            Скрыть
          </button>
        </div>
      ...
      {testResult && (
        <div
          role="status"
          className="bg-blue-50 dark:bg-blue-950/40 border border-blue-500 rounded p-3 mb-4 flex items-center justify-between"
        >
          <span>{testResult.message}</span>
          <button
            onClick={() => setTestResult(null)}
            className="ml-3 bg-transparent border-none cursor-pointer underline text-sm text-gray-600 dark:text-slate-400"
          >
            Скрыть
          </button>
        </div>
      ...
      <Modal open={showCreateForm} onClose={() => setShowCreateForm(false)} title="Создать вебхук">
        <form onSubmit={handleCreate}>
          ...
          <div className="mb-4">
            <fieldset>
              <legend className="text-sm font-medium text-gray-700 dark:text-slate-300">Типы событий *</legend>
