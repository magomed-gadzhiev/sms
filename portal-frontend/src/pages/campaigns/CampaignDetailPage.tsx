...
function StatCard({ label, value, subtext, color = 'text-gray-900 dark:text-slate-100' }: StatCardProps) {
  return (
...
export function CampaignDetailPage() {
  ...
  usePageTitle(campaign?.name ?? 'Рассылка');
      ...
      <div role="status" aria-label="Загрузка рассылки" className="p-8">
        <div className="animate-pulse space-y-4 max-w-2xl">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            {[...Array(5)].map((_, i) => <div key={i} className="h-20 bg-gray-100 dark:bg-slate-800 rounded" />)}
          </div>
        </div>
      </div>
    );
  ...
  if (error) {
    return (
      <div className="p-8">
        <p role="alert" className="text-red-600 dark:text-red-400 mb-4">{error}</p>
    ...
    <div>
      ...
      {(!hasABTest || activeTab === 'overview') && (
        <>
          ...
          {stats?.per_variant && stats.per_variant.length > 0 && !hasABTest && (
            <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-lg overflow-hidden mb-6">
              <div className="px-4 py-3 border-b border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-950">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <thead className="bg-gray-50 dark:bg-slate-950 border-b border-gray-200 dark:border-slate-700">
                    <tr>
                      <th className="px-4 py-3 text-left font-medium text-gray-700 dark:text-slate-300">Вариант</th>
                      <th className="px-4 py-3 text-left font-medium text-gray-700 dark:text-slate-300">Отправлено</th>
                      <th className="px-4 py-3 text-left font-medium text-gray-700 dark:text-slate-300">Доставлено</th>
                      <th className="px-4 py-3 text-left font-medium text-gray-700 dark:text-slate-300">Доставляемость</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100 dark:divide-slate-800">
                    {stats.per_variant.map((pv) => (
                      <tr key={pv.variant_id} className="hover:bg-gray-50 dark:hover:bg-slate-800">
                        <td className="px-4 py-3 font-medium text-gray-900 dark:text-slate-100">{pv.variant_name}</td>
                        <td className="px-4 py-3 text-gray-800 dark:text-slate-200">{(pv.sent ?? 0).toLocaleString()}</td>
                        <td className="px-4 py-3 text-green-600 dark:text-green-400">{(pv.delivered ?? 0).toLocaleString()}</td>
                        <td className="px-4 py-3 text-gray-800 dark:text-slate-200">{((pv.delivery_rate ?? 0) * 100).toFixed(1)}%</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      {hasABTest && activeTab === 'ab_results' && (
        <div className="space-y-6">
          ...
          {campaign.ab_config && (
            <div className="bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800 rounded-lg p-4 text-sm">
              <h4 className="font-semibold text-blue-800 dark:text-blue-300 mb-2">Конфигурация теста</h4>
          ...
          <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-lg overflow-hidden">
            <div className="px-4 py-3 border-b border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-950">
              <h3 className="font-medium text-gray-900 dark:text-slate-100">Сравнение вариантов</h3>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <thead className="bg-gray-50 dark:bg-slate-950 border-b border-gray-200 dark:border-slate-700">
                  <tr>
                    <th className="px-4 py-3 text-left font-medium text-gray-700 dark:text-slate-300">Вариант</th>
                    <th className="px-4 py-3 text-left font-medium text-gray-700 dark:text-slate-300">Доля</th>
                    <th className="px-4 py-3 text-left font-medium text-gray-700 dark:text-slate-300">Отправлено</th>
                    <th className="px-4 py-3 text-left font-medium text-gray-700 dark:text-slate-300">Доставлено</th>
                    <th className="px-4 py-3 text-left font-medium text-gray-700 dark:text-slate-300">Ошибки</th>
                    <th className="px-4 py-3 text-left font-medium text-gray-700 dark:text-slate-300">Доставляемость</th>
                    <th className="px-4 py-3 text-left font-medium text-gray-700 dark:text-slate-300">Статус</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100 dark:divide-slate-800">
                  {campaign.variants!.map((v) => {
                    ...
                    return (
                      <tr
                        key={v.id}
                        className={`hover:bg-gray-50 dark:hover:bg-slate-800 ${isWinner ? 'bg-green-50 dark:bg-green-950/40' : ''}`}
                      >
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            <span className="font-medium text-gray-900 dark:text-slate-100">{v.name}</span>
                            {v.is_control && (
                              <Badge variant="default">Контроль</Badge>
                            )}
                          </div>
                        </td>
                        <td className="px-4 py-3 text-gray-600 dark:text-slate-400">{v.percentage}%</td>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-gray-700 dark:text-slate-300 text-xs font-medium w-12 text-right">
