...
export function CampaignWizardPage() {
  ...
  return (
    <div>
      ...
      {error && (
        <div role="alert" className="bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-300 rounded p-3 mb-4 text-sm">
          {error}
        </div>
      )}
      ...
      <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-lg p-6">
        ...
        {step === 'message' && (
          <div className="space-y-5 max-w-xl">
            <h3 className="text-lg font-medium text-gray-900 dark:text-slate-100">Сообщение</h3>

            {/* Template picker — required */}
            {/* Textarea — primary input per spec */}
            <div className="space-y-1">
              <label htmlFor="msg-text" className="text-sm font-medium text-gray-700 dark:text-slate-300">
                Текст сообщения *
              </label>
              <textarea
                id="msg-text"
                ...
                rows={3}
                className="w-full rounded border border-gray-300 dark:border-slate-600 px-3 py-2 text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 focus-visible:border-primary"
            ...
            <div className="pt-2 border-t border-gray-200 dark:border-slate-700">
              <label className="flex items-center gap-3 cursor-pointer select-none">
                ...
                >
                  <span className={`inline-block h-4 w-4 transform rounded-full bg-white dark:bg-slate-900 transition-transform ${abEnabled ? 'translate-x-6' : 'translate-x-1'}`} />
                </button>
                <span className="text-sm font-medium text-gray-700 dark:text-slate-300">A/B тестирование</span>
              </label>
            </div>

            {/* A/B panel */}
            {abEnabled && (
              <div className="space-y-4 pl-4 border-l-2 border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-950/40/30 rounded-r p-4">
                <h4 className="text-sm font-semibold text-gray-800 dark:text-slate-200">Вариант B</h4>

                {/* Textarea — primary input for variant B */}
                <div className="space-y-1">
                  <label htmlFor="msg-text-b" className="text-sm font-medium text-gray-700 dark:text-slate-300">
                    Текст варианта B *
                  </label>
                  <textarea
                    id="msg-text-b"
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium text-gray-700 dark:text-slate-300">
                    ...
                    <span className="text-blue-600 dark:text-blue-400 font-semibold">{abSplitPercent}%</span>
                  </label>
                ...
                <fieldset>
                  <legend className="text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">Метрика победителя</legend>
                  <div className="space-y-2">
                    {[
        ...
        {step === 'audience' && (
          <div className="space-y-5 max-w-xl">
            <h3 className="text-lg font-medium text-gray-900 dark:text-slate-100">Аудитория</h3>

            {contactListsError && (
              <p className="text-xs text-red-600 dark:text-red-400">{contactListsError}</p>
            )}
            <Select
            ...
            {contactListId && (
              <>
                <button
                  ...
                  className="text-sm text-blue-600 dark:text-blue-400 hover:text-blue-800 underline-offset-2 hover:underline flex items-center gap-1"
                >
                ...
                {showFilters && segments && (
                  <div className="space-y-4 pl-4 border-l-2 border-gray-200 dark:border-slate-700">
                    {segments.countries.length > 0 && (
                      <div className="space-y-2">
                        <label className="text-sm font-medium text-gray-700 dark:text-slate-300">Исключить страны</label>
                    ...
                    {segments.operators.length > 0 && (
                      <div className="space-y-2">
                        <label className="text-sm font-medium text-gray-700 dark:text-slate-300">Исключить операторов</label>
                        <div className="flex flex-wrap gap-2">
                ...
                {(() => {
                  ...
                  return (
                    <div className="bg-gray-50 dark:bg-slate-950 rounded-lg p-3 text-sm">
                      <span>
                        Контактов к отправке: <strong>{total.toLocaleString()}</strong>
                        {hasFilters && <span className="text-gray-500 dark:text-slate-400 ml-1">(фильтры применяются при запуске)</span>}
                        {abEnabled && (
                          <span className="block text-gray-500 dark:text-slate-400 mt-1">
            ...
            {validationErrors.audience && (
              <p className="text-sm text-red-600 dark:text-red-400">Выберите контактную базу.</p>
            )}
          </div>
        )}
        ...
        {step === 'schedule' && (
          <div className="space-y-5 max-w-xl">
            <h3 className="text-lg font-medium text-gray-900 dark:text-slate-100">Расписание</h3>

            <fieldset>
              <legend className="text-sm font-medium text-gray-700 dark:text-slate-300 mb-3">Время отправки</legend>
              <div className="space-y-3">
                <label className="flex items-center gap-3 cursor-pointer">
                  ...
                  />
                  <div>
                    <span className="text-sm font-medium text-gray-700 dark:text-slate-300">Сейчас</span>
                    <p className="text-xs text-gray-500 dark:text-slate-400">Рассылка начнётся сразу после подтверждения</p>
                  </div>
                </label>
                <label className="flex items-center gap-3 cursor-pointer">
                  <input
                  ...
                  />
                  <div>
                    <span className="text-sm font-medium text-gray-700 dark:text-slate-300">Позже</span>
                    <p className="text-xs text-gray-500 dark:text-slate-400">Выберите дату и время начала</p>
                  </div>
                </label>
              </div>
            </fieldset>
            ...
            {validationErrors.schedule && (
              <p className="text-sm text-red-600 dark:text-red-400">Укажите корректную дату и время отправки.</p>
            )}
          </div>
        )}
        ...
        {step === 'confirm' && (
          <div className="space-y-5 max-w-xl">
            <h3 className="text-lg font-medium text-gray-900 dark:text-slate-100">Подтверждение</h3>

            {/* Campaign name */}
            <div className="space-y-1">
              <label className="text-sm font-medium text-gray-700 dark:text-slate-300">Название рассылки</label>
              {editingName ? (
                <Input
                  type="text"
                  value={campaignName}
                ...
                <div className="flex items-center gap-2">
                  <span className="text-sm text-gray-900 dark:text-slate-100">{campaignName}</span>
                  <button
                    type="button"
            ...
            {/* Summary */}
            <dl className="bg-gray-50 dark:bg-slate-950 rounded-lg divide-y divide-gray-200 dark:divide-slate-700 text-sm overflow-hidden border border-gray-200 dark:border-slate-700">
              <div className="p-3 flex justify-between items-start gap-2">
                <div className="min-w-0">
                  <dt className="text-gray-500 dark:text-slate-400 text-xs mb-1">Сообщение</dt>
                  <dd className="text-gray-900 dark:text-slate-100 truncate">
                    {(messageText || selectedTemplate?.body || '—').slice(0, 100)}
                    {(messageText || selectedTemplate?.body || '').length > 100 ? '…' : ''}
                  </dd>
                  <dd className="text-gray-500 dark:text-slate-400 text-xs mt-0.5">
                    Отправитель: {senderNames.find((s) => s.id === senderNameId)?.name || '—'}
                  </dd>
                </div>
                <button type="button" onClick={() => setStep('message')}
                  className="shrink-0 text-xs text-blue-600 dark:text-blue-400 hover:underline">изменить</button>
              </div>
              ...
              {abEnabled && (() => {
                ...
                return (
                  <div className="p-3 flex justify-between items-start gap-2">
                    <div className="min-w-0">
                      <dt className="text-gray-500 dark:text-slate-400 text-xs mb-1">A/B тестирование</dt>
                      <dd className="text-gray-900 dark:text-slate-100 truncate">{variantBLabel}</dd>
                      <dd className="text-gray-500 dark:text-slate-400 text-xs mt-0.5">
              ...
              <div className="p-3 flex justify-between items-start gap-2">
                <div className="min-w-0">
                  <dt className="text-gray-500 dark:text-slate-400 text-xs mb-1">Аудитория</dt>
              ...
              <div className="p-3 flex justify-between items-start gap-2">
                <div className="min-w-0">
                  <dt className="text-gray-500 dark:text-slate-400 text-xs mb-1">Расписание</dt>
                  <dd className="text-gray-900 dark:text-slate-100">
            ...
            <div className="bg-gray-50 dark:bg-slate-950 rounded-lg p-4 border border-gray-200 dark:border-slate-700">
              <h4 className="text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">Предварительная стоимость</h4>
              {costLoading ? (
                ...
                <div className="space-y-1 text-sm">
                  <div className="flex justify-between">
                    <span className="text-gray-500 dark:text-slate-400">Получателей</span>
                    <span className="font-medium">{costEstimate.recipients.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-500 dark:text-slate-400">Частей SMS</span>
                  ...
                  <div className="flex justify-between border-t border-gray-200 dark:border-slate-700 pt-1 mt-1">
                    <span className="text-gray-700 dark:text-slate-300 font-medium">Итого</span>
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-gray-500 dark:text-slate-400">Баланс</span>
                    <span className={costEstimate.balance_sufficient ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}>
                ...
                <p className="text-sm text-gray-400 dark:text-slate-500">Не удалось рассчитать стоимость</p>
              )}
            </div>
          </div>
        )}
