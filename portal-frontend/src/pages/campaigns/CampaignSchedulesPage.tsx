...
export function CampaignSchedulesPage() {
  usePageTitle('Повторяющиеся рассылки');
  ...
  const columns: Column<CampaignSchedule>[] = [
    {
      key: 'name',
      header: 'Название',
      render: (s) => <span className="font-medium text-gray-900 dark:text-slate-100">{s.name}</span>,
    },
    {
      ...
      render: (s) => (
        <span className="text-gray-600 dark:text-slate-400 text-sm">
          {s.template_campaign_name || <span className="text-gray-400 dark:text-slate-500 italic">удалена</span>}
        </span>
      ),
    },
    {
      ...
      render: (s) => (
        <span>
          ...
          {s.frequency === 'custom' && s.cron_expression && (
            <code className="ml-1 text-xs text-gray-400 dark:text-slate-500 font-mono">({s.cron_expression})</code>
          )}
        </span>
      ),
    },
    {
      ...
      responsive: true,
      render: (s) => <span className="text-gray-500 dark:text-slate-400 text-sm">{formatDate(s.last_run_at)}</span>,
    },
    {
      key: 'next_run_at',
  ...
  return (
    <div>
      ...
      {toggleError && (
        <p className="text-red-600 dark:text-red-400 mb-4 text-sm">{toggleError}</p>
      )}
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          <div>
            ...
            <input
              type="text"
              className="w-full border border-gray-300 dark:border-slate-600 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              value={form.name}
          ...
          <div>
            ...
            {campaignsLoading ? (
              <select disabled className="w-full border border-gray-300 dark:border-slate-600 rounded-md px-3 py-2 text-sm bg-gray-50 dark:bg-slate-950 text-gray-400 dark:text-slate-500">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Частота</label>
            <select
              className="w-full border border-gray-300 dark:border-slate-600 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
