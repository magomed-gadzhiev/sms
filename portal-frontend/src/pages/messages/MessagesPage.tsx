...
export function MessagesPage() {
  ...
  // Clear selection whenever filters/page change (rows on screen change).
  useEffect(() => { setSelectedIds(new Set()); }, [appliedFilters, page, pageSize, sort]);

  const pageIds = (data?.messages ?? []).map((m) => m.id);
  ...
  const exportSelectedCsv = () => {
    ...
    a.href = url;
    a.download = `messages_selected_${new Date().toISOString().slice(0, 19).replace(/[:T]/g, '-')}.csv`;
    a.click();
  ...
  return (
    <div>
      ...
      {error && <div role="alert" className="text-red-600 dark:text-red-400 mb-3 text-sm">Ошибка: {error}</div>}

      <div className="flex items-center justify-between mb-2">
        <span className="text-sm text-gray-500 dark:text-slate-400" aria-live="polite" aria-atomic="true">
          {loading ? (
            <span role="status" className="inline-flex items-center gap-1.5">
              <svg className="animate-spin w-4 h-4 text-gray-400 dark:text-slate-500" fill="none" viewBox="0 0 24 24" aria-hidden="true">
        ...
        <div className="flex gap-2">
          {exportStatus && exportStatus !== 'ready' && (
            <span role="status" className="text-sm text-gray-400 dark:text-slate-500 self-center">Экспорт...</span>
          )}
          <div className="flex flex-col items-end">
            ...
            {exportError && (
              <p role="alert" className="text-sm text-red-600 dark:text-red-400 mt-1">{exportError}</p>
            )}
          </div>
          <ColumnConfigurator
            columns={columnDefs}
