...
export function MessageDetailPage() {
  ...
  usePageTitle(message?.destination ? `Сообщение → ${message.destination}` : 'Сообщение');
  ...
  if (loading) {
    return <div className="animate-pulse p-8 text-center text-gray-500 dark:text-slate-400">Загрузка...</div>;
  }
  ...
  return (
    <div className="max-w-3xl">
      ...
      {/* Detail card */}
      <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-lg p-6 mb-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <span className="text-sm text-gray-500 dark:text-slate-400">ID сообщения</span>
            <p className="font-mono text-sm mt-0.5 break-all">{message.message_id}</p>
          </div>
          <div>
            <span className="text-sm text-gray-500 dark:text-slate-400">Статус</span>
            <div className="mt-0.5">
              <Badge variant={statusVariant}>{statusLabel}</Badge>
            </div>
          </div>
          <div>
            <span className="text-sm text-gray-500 dark:text-slate-400">Отправитель</span>
            <p className="text-sm mt-0.5">{message.source || '--'}</p>
          </div>
          <div>
            <span className="text-sm text-gray-500 dark:text-slate-400">Получатель</span>
            <p className="text-sm mt-0.5">{message.destination}</p>
          </div>
          <div>
            <span className="text-sm text-gray-500 dark:text-slate-400">Сегменты</span>
            <p className="text-sm mt-0.5">{message.segment_count}</p>
          </div>
          <div>
            <span className="text-sm text-gray-500 dark:text-slate-400">Создано</span>
            <p className="text-sm mt-0.5">{formatTimestamp(message.created_at)}</p>
          </div>
          {message.encoding && (
            <div>
              <span className="text-sm text-gray-500 dark:text-slate-400">Кодировка</span>
              <p className="text-sm mt-0.5 font-mono">{message.encoding}</p>
            </div>
          )}
          {message.external_id && (
            <div>
              <span className="text-sm text-gray-500 dark:text-slate-400">Внешний ID</span>
              <p className="text-sm mt-0.5 font-mono break-all">{message.external_id}</p>
            </div>
          )}
          {message.provider_name && (
            <div>
              <span className="text-sm text-gray-500 dark:text-slate-400">Провайдер</span>
              <p className="text-sm mt-0.5">{message.provider_name}</p>
            </div>
          )}
          {message.route_name && (
            <div>
              <span className="text-sm text-gray-500 dark:text-slate-400">Маршрут</span>
              <p className="text-sm mt-0.5">{message.route_name}</p>
            </div>
          )}
          {message.smpp_message_id && (
            <div>
              <span className="text-sm text-gray-500 dark:text-slate-400">SMPP ID</span>
          ...
          {(message.retry_count !== undefined && message.retry_count > 0) && (
            <div>
              <span className="text-sm text-gray-500 dark:text-slate-400">Попытки</span>
          ...
          {message.scheduled_at && (
            <div>
              <span className="text-sm text-gray-500 dark:text-slate-400">Запланировано</span>
              <p className="text-sm mt-0.5">{formatTimestamp(message.scheduled_at)}</p>
            </div>
          )}
          {message.expired_at && (
            <div>
              <span className="text-sm text-gray-500 dark:text-slate-400">Истекает</span>
        ...
        {message.text && (
          <div className="mt-4 pt-4 border-t border-gray-100 dark:border-slate-800">
            <span className="text-sm text-gray-500 dark:text-slate-400">Текст сообщения</span>
            <p className="text-sm mt-1 whitespace-pre-wrap bg-gray-50 dark:bg-slate-950 rounded p-3">{message.text}</p>
          </div>
        )}
        {message.status_message && (
          <div className="mt-4 pt-4 border-t border-gray-100 dark:border-slate-800">
            <span className="text-sm text-red-500 dark:text-red-400">Ошибка</span>
            <p className="text-sm mt-1 text-red-700 dark:text-red-300">{message.status_message}</p>
          </div>
        )}
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-lg p-6">
        <h2 className="text-base font-semibold text-gray-900 dark:text-slate-100 mb-5">Статус доставки</h2>
            ...
            <div key={step.key} className="flex-1 flex flex-col items-center relative">
                ...
                className={`relative z-10 w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${
                  step.done
                    ? step.failed
                      ...
                      : 'bg-green-100 dark:bg-green-900/40 text-green-600 dark:text-green-400 ring-2 ring-green-400'
                    : 'bg-gray-100 dark:bg-slate-800 text-gray-400 dark:text-slate-500 ring-2 ring-gray-200'
                }`}
              >
              ...
              <span
                className={`mt-2 text-xs font-medium ${
                  step.done ? (step.failed ? 'text-red-600 dark:text-red-400' : 'text-green-700 dark:text-green-300') : 'text-gray-400 dark:text-slate-500'
                }`}
              >
                {step.label}
              </span>
      ...
      {message.dlr && (
        <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-lg p-6 mt-6">
          <h2 className="text-base font-semibold text-gray-900 dark:text-slate-100 mb-4">Ответ оператора (DLR)</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <span className="text-sm text-gray-500 dark:text-slate-400">Статус оператора</span>
              <div className="mt-0.5">
            ...
            {message.dlr.err !== 0 && (
              <div>
                <span className="text-sm text-gray-500 dark:text-slate-400">Код ошибки</span>
                <p className="text-sm mt-0.5 font-mono text-red-700 dark:text-red-300">{message.dlr.err}</p>
              </div>
            )}
            {message.dlr.submit_date && (
              <div>
                <span className="text-sm text-gray-500 dark:text-slate-400">Принято оператором</span>
                <p className="text-sm mt-0.5">{formatTimestamp(message.dlr.submit_date)}</p>
              </div>
            )}
            {message.dlr.done_date && (
              <div>
                <span className="text-sm text-gray-500 dark:text-slate-400">Время финального статуса</span>
                <p className="text-sm mt-0.5">{formatTimestamp(message.dlr.done_date)}</p>
              </div>
            )}
            {message.dlr.receipted_message_id && (
              <div className="sm:col-span-2">
                <span className="text-sm text-gray-500 dark:text-slate-400">ID оператора</span>
          ...
          {message.dlr.text && (
            <div className="mt-4 pt-4 border-t border-gray-100 dark:border-slate-800">
              <span className="text-sm text-gray-500 dark:text-slate-400">Текст от оператора</span>
              <p className="text-sm mt-1 whitespace-pre-wrap bg-gray-50 dark:bg-slate-950 rounded p-3 font-mono">{message.dlr.text}</p>
            </div>
          )}
        </div>
      )}
      ...
      {message.billing && (
        <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-lg p-6 mt-6">
          <h2 className="text-base font-semibold text-gray-900 dark:text-slate-100 mb-4">Биллинг</h2>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <span className="text-sm text-gray-500 dark:text-slate-400">Сегментов</span>
              <p className="text-sm mt-0.5 font-semibold">{message.billing.segment_count}</p>
            </div>
            <div>
              <span className="text-sm text-gray-500 dark:text-slate-400">Цена за сегмент</span>
              <p className="text-sm mt-0.5 font-semibold">{message.billing.price_per_segment} ₽</p>
            </div>
            <div>
              <span className="text-sm text-gray-500 dark:text-slate-400">Итого</span>
              <p className="text-sm mt-0.5 font-semibold text-blue-700 dark:text-blue-300">{message.billing.total_amount} ₽</p>
            </div>
          </div>
