...
- ~~**18 других .proto файлов не нормализованы** — текущий baseline-regen покрывает только auth и client (целевая работа A.1/D.1/RotateAPIKey). Остальные 18 (cascade, billing, tarification, webhook, audit, company, link, messaging, routing, provider, smpp, analytics, contact, campaign, template, sms, network_analytics, sender-name) остаются на разных версиях protoc-gen-go (v1.34.1, v1.36.11) и protoc (v3.21.12, v4.25.1, v6.31.1). `network_analytics` и `sender-name` вообще не имеют сгенерированных `.pb.go`. Если когда-нибудь захочется унифицировать — отдельная задача (Variant B/C из стратегического разговора), не блокирующая~~ — [DONE] commits `0cbd11d` (C1: cleanup 6 stale duplicate dirs), `1ea9051` (C2: restore client_provider.proto + extend registry to 21 targets), `076a2c6` (C3: full regen всех + cleanup ClientProvider* duplicates в provider.proto). Все 21 .proto унифицированы на protoc v6.31.1, protoc-gen-go v1.36.11, protoc-gen-go-grpc v1.6.1. RPC counts preserved (routing 40→40, cascade 16→16). Memory rule no-smart-routing подтвержден removal'ом 12 dead mock-functions. Признана и исправлена ошибка C2 (был излишне ограничивал — отверг A2 на основе stale .pb.go вместо current source).
...
- ~~`updates_fields` test coverage gap для IsReseller/MaxSubAccounts unchanged-when-nil~~ — [DONE] commit `7501b1e` (2026-05-03). Создан internal/gateway/admin/handlers/clients_test.go (~230 строк). 7 тестов покрывают handler pass-through (NilFieldsPreserveAsNil regression-guard для housekeeping #2 + IsReseller/MaxSubAccounts gap), explicit-false/true сценарии, malformed JSON→400, gRPC NotFound/InvalidArgument маппинг, CreateClient.Validate() reseller-без-слотов short-circuit. Reviewer APPROVED 1 итерация + minor nit (fmt.Sprint assertion).
