# Aggregator Network Dashboard — Phase 3 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Give aggregators a dashboard showing their network state at a glance — balances, traffic, delivery rate, moderation queue, top sub-accounts, and alerts.

**Architecture:** Single backend endpoint `GET /portal/v1/reseller/dashboard` uses parallel goroutines (same pattern as existing `dashboard.go`) to fetch data from gRPC services and SQL. Frontend renders 7 metric cards. Revenue card shows placeholder until Phase 4 (tariffs).

**Tech Stack:** Go 1.24.0, gorilla/mux, pgx/v5, gRPC (billingv1, analyticsv1, clientv1), TypeScript 5.7, React 19, Tailwind CSS 4.2

---

## Task 1: Backend — Reseller dashboard handler

**Files:**
- Create: `internal/gateway/portal/handlers/reseller_dashboard.go`

- [ ] **Step 1: Create reseller dashboard handler**
...
func (h *ResellerDashboardHandlers) GetResellerDashboard(w http.ResponseWriter, r *http.Request) {
	...
	go func() {
		...
		// Sort by sent desc (simple selection for top 5)
...
```bash
git add internal/gateway/portal/handlers/reseller_dashboard.go
git commit -m "feat(portal): add reseller network dashboard handler"
```

---

## Task 2: Backend — Register dashboard route

**Files:**
- Modify: `internal/gateway/portal/router/router.go`
- Modify: `cmd/portal-gateway/main.go`

- [ ] **Step 1: Add handler param to SetupRouter**

In router.go, after `resellerTemplateHandlers *handlers.ResellerTemplateHandlers,` add:
...
In router.go, in the reseller routes section, add before the moderation counts route:
...
After `resellerTemplateHandlers` initialization, add:
...
Pass to SetupRouter after `resellerTemplateHandlers,`:
...
git commit -m "feat(portal): register reseller dashboard route"
```

---

## Task 3: Frontend — NetworkDashboardPage

**Files:**
- Create: `portal-frontend/src/pages/network/NetworkDashboardPage.tsx`
...
## Task 4: Frontend — Wire dashboard and update NetworkLayout

**Files:**
- Modify: `portal-frontend/src/App.tsx`
- Modify: `portal-frontend/src/components/layout/NetworkLayout.tsx`

- [ ] **Step 1: Add dashboard route, update layout nav**
- [ ] **Step 2: Verify builds and commit**
