...
- Integration with existing SendMessage flow
...
**Rendering flow:** Client Gateway calls `RenderTemplate(template_id, variables)` → gets rendered text → passes it to Messaging Service as regular `text`. Messaging Service has no knowledge of templates.
...
CREATE TABLE template_audit_log (
    ...
    action VARCHAR(20) NOT NULL
        CHECK (action IN ('created', 'updated', 'approved', 'rejected')),
    old_body TEXT,
    new_body TEXT,
    actor_id UUID,
...
**UpdateTemplate:** `id`, `client_id`, `name` (optional), `body` (optional) → updates template. If `body` changes, status resets to `draft`, writes audit log with old/new body.
...
**RenderTemplate:** `template_id`, `client_id`, `variables` (map<string, string>) → validates template is `approved`, validates all required variables are provided, performs substitution, returns `rendered_text` and `template_name`.

**GetTemplateAuditLog:** `template_id` → returns list of audit entries ordered by created_at DESC.
...
| GET | `/api/v1/templates` | List own templates (filter by status) |
...
| Method | Path | Description |
|--------|------|-------------|
| GET | `/admin/v1/templates` | List templates (filter by client_id, status) |
...
| GET | `/admin/v1/templates/{id}/audit` | Template audit log |
...
4. `template_id` in SendMessage is stored in the `messages` table for traceability (new nullable column `template_id`)
...
4. `strings.NewReplacer` for substitution: `{{name}}` → value from map
...
- Route registration: `/api/v1/templates/*` in `router.go`
- **Modify `handlers/sms.go`:** add template rendering logic to `SendSMS`
...
- Route registration: `/admin/v1/templates/*` in `router.go`
- `TEMPLATE_SERVICE_ADDR` environment variable

### Messaging Service — No Changes
Receives `text` as before. No knowledge of templates.
...
### Dockerfile
Add protoc generation step for `api/proto/template/template.proto` in `service-base.Dockerfile`.
