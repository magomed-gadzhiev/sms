...
# В контейнере
migrate -path migrations -database "postgres://smpp:smpp_password@postgres:5432/smpp_db?sslmode=disable" up
```

3. Проверить статус миграций:
```bash
migrate -path migrations -database "postgres://smpp:smpp_password@postgres:5432/smpp_db?sslmode=disable" version
```
