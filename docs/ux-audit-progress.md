...
TC-AUTH-1..10 (PASS): без cookie 401; не-uuid cookie 401; несуществующий uuid 401; невалидный JWT 401; fake `sk_live_*` 401; публичные endpoints (`/plans`, `/billing/top-up/callback`, `/health`) 200.

TC-RBAC-1..17 (PASS): user/aggregator/sub → /admin/v1/* → 403 "Admin access required"; user → /reseller/* → 401 "доступ только для агрегаторов" (regression-проверка BUG-82); sub → /reseller/* → 401 (regression-проверка); aggregator → /reseller/dashboard → 200; user/aggregator → /portal/v1/admin/channels → 403 "Доступ запрещён" (AdminRoleMiddleware); admin → 200; user/sub → POST /sub-accounts → 403 "client is not a reseller"; aggregator → POST /sub-accounts → 200; все роли → /portal/v1/dashboard → 200 (own); все роли → /portal/v1/audit-log → 200 (own, кроме admin → 401 "Клиент не найден" т.к. admin без client_id — корректно).

TC-IDOR-1..N (PASS с info-disclosure observation): sub → GET/PUT/DELETE user's api-key/{id} → 404/403/403; sub → webhook → 404 (GET /webhooks/{id} вообще не зарегистрирован — gorilla 404 plain-text, OBS-5 этапа 25 reproduced); sub → template → 404; sub → sender-name → 404 (с валидным body); cross-aggregator: Demo-Reseller → /sub-accounts/{r2-sub-id} GET/PUT/DELETE/transfer → 404 PASS.

TC-PRIV-ESC-1..5 (PASS, кроме PRIV-4 OBS): user/agg/sub → /admin/v1/clients/{any}/rate-limits, /freeze, POST /users, POST /clients → все 403 "Admin access required"; user → PUT /portal/profile с email="admin@example.com" → 200 "Профиль обновлён", БД не изменилась — silent ignore (OBS-2 этапа 27 reproduced).

TC-CSRF-1..5 (PASS): POST /api-keys без X-CSRF-Token → 403 CSRF_VALIDATION_FAILED; POST /sub-accounts без CSRF → 403; PUT /profile без CSRF → 403; GET /dashboard → 200 (read = no-CSRF requirement); POST /campaigns с api-key без CSRF → 201 (правильный bypass для Bearer auth).

**TC-SCOPE-1..3 — 1 CRITICAL ВОСПРОИЗВЕДЕНО**: api-key c scopes=["messages:read"] (read-only) делает POST/PUT/DELETE /campaigns → 201/200/204. Полное privilege escalation.

TC-AGG-1..7 (cross-aggregator + partner_id leak): sub-account isolation Demo-Reseller ↔ AuditReseller2 PASS. **OBS-3 этапа 27 reproduced**: reseller2 → /reseller/statistics → 200 с теми же KPI (value=2 / 1) что Demo-Reseller — оба читают partner_id=0 глобальный bucket; SQL `SELECT partner_id, count(*) FROM network_stats_hourly` показывает только 1 row partner_id=0 на стенде. Архитектурный leak подтверждён.

**0 фиксов, 1 CRITICAL эскалирован, 5 OBSERVATION'ов (1 новая, 4 повторных подтверждения от прошлых этапов).**

[BUG LIST]

BUG-83: API-key scope NOT enforced — read-only key (`messages:read`) делает POST/PUT/DELETE /campaigns успешно — Severity: **CRITICAL** — Категория: Security / Privilege escalation / Authorization bypass — **ЭСКАЛИРОВАН**
  Шаги: 1) user POST /portal/v1/api-keys c body `{"name":"audit-final","scopes":["messages:read"]}` → 201 + key `sk_live_…`. 2) `curl -X POST -H "Authorization: Bearer sk_live_…" /portal/v1/campaigns -d '{"name":"hack","contact_list_id":"…","template_id":"…"}'` → **201 created**. 3) `curl -X PUT -H "Authorization: Bearer …" /portal/v1/campaigns/{id} -d '{"name":"hack-renamed"}'` → **200 OK**. 4) `curl -X DELETE -H "Authorization: Bearer …" /portal/v1/campaigns/{id}` → **204**.
  Ожидалось: 403 FORBIDDEN "scope messages:write required" (или 401) для всех 3 write-операций; только GET /campaigns должен возвращать 200.
  Получалось: все 3 write-операции принимаются как от обычной user-сессии. БД-фиксация: `INSERT INTO campaigns ... RETURNING id` действительно создаёт запись.
  Корень: `internal/gateway/portal/middleware/api_key_auth.go::APIKeyAuthMiddleware` валидирует токен через `authClient.ValidateToken`, но НЕ запрашивает scopes (`api_key_scopes` таблица не читается), не передаёт scopes в context, и нет middleware/handler-checks вида `if method ∈ {POST,PUT,DELETE} && !HasScope(ctx, "messages:write") → 403`. UI выставляет scopes при создании ключа (`portal-frontend/src/pages/api-keys/APIKeysPage.tsx:11-17`: AVAILABLE_SCOPES = messages:send|messages:read|webhooks:manage|analytics:read|sub-accounts:manage), seed создаёт ключи с scopes — но во всём portal-gateway нет ни одного callsite `domain.APIKey.HasScope(...)` (grep подтверждает: `HasScope` определён только в `internal/services/auth/domain/api_key.go:74`, используется только в его же тестах).
  Доказательство:
    - SQL `SELECT scope FROM api_key_scopes WHERE api_key_id='…'` → `messages:read`.
    - TC-SCOPE-1c..1e: 201/200/204 для POST/PUT/DELETE.
    - Code: `grep -rn "HasScope\|RequireScope" internal/gateway/` → 0 hits.
  Импакт: 1) любой read-only ключ (создаваемый UI с scope=messages:read для аналитики/мониторинга) выполняет write-операции на campaigns — privilege escalation. 2) Радиус ограничен: API-key auth применяется только на `/portal/v1/campaigns/*` subrouter (TC проверка: api-key → /dashboard, /messages, /sub-accounts, /api-keys, /webhooks → 401 "Сессия не найдена", т.е. остальной portal требует session). На /campaigns/* доступны: CreateCampaign, UpdateCampaign, DeleteCampaign, LaunchCampaign, PauseCampaign, ResumeCampaign, CancelCampaign, SetVariants, SetABConfig, SelectWinner, SetRetryConfig, RetryFailed, ExportReport — все write-операции. 3) Производственный риск: read-only intent утечёт write-возможность в любую интеграцию пользователя (мониторинг/аналитика-боты, BI-выгрузки, dashboard'ы), которые держат `sk_live_*` в env vars. В конечном счёте read-only ключ может запустить кампанию (LaunchCampaign), потратить баланс клиента или удалить чужие чёрные кампании.
  Архитектурный фикс (требует решения пользователя — 3 варианта):
    a) **Полная реализация scope enforcement**: расширить `authv1.ValidateTokenResponse` → `repeated string scopes`; вернуть `api_key_scopes.scope[]` из auth-service ValidateToken; в `APIKeyAuthMiddleware` положить scopes в context; добавить middleware `RequireScope(scope string)` который читает context и сравнивает; на `/portal/v1/campaigns/*` повесить per-method guards (GET → "messages:read", POST/PUT → "messages:send", DELETE → одно из двух или новая "messages:delete"). Затрагивает: auth-service (proto + handler + repo), portal-gateway middleware, router. Требует миграцию scope-имён, т.к. UI и БД seed используют `messages:send`/`messages:read` без `messages:write` (нет write-scope как такового). Объём работы — отдельная инициатива со spec.
    b) **Жёсткое временное закрытие write через API-key**: в `APIKeyAuthMiddleware` или в кастомном `APIKeyReadOnlyMiddleware` отказывать на `r.Method != "GET"`. Закрывает privilege escalation немедленно ценой отключения programmatic write через API. Не ломает ни одного известного use-case (UI всегда session-auth; программные write-интеграции не задокументированы — но возможны у пользователей, которые их написали в обход контракта).
    c) **Откатить sessionOrAPIKey middleware с /campaigns/***: вернуть API-key auth обратно к session-only (как для /messages, /api-keys и т.д.) — закрывает privilege escalation полностью, возвращает к консистентному поведению "API-key пока не работает нигде". Цена: ломает декларированный контракт `/campaigns/*` поддерживает Bearer.
  По §5.4 spec'a: Security-баг (privilege escalation) + архитектурное решение, затрагивает auth-service и portal-gateway → стоп, эскалация. Не фикшу в этом этапе.

[OBSERVATION-1 этапа 28] Info-disclosure: PUT/DELETE /api-keys/{cross-tenant-id} → 403 "API key does not belong to user" вместо 404 — Severity: COSMETIC — Категория: HTTP semantic / Info-disclosure
  Sub → PUT/DELETE /portal/v1/api-keys/{user-id} → 403 "API key does not belong to user". GET того же id → 404 "API ключ не найден". Inconsistent: GET скрывает существование, PUT/DELETE раскрывают (атакующий может перебором узнать какие api-key id'ы существуют в системе через PUT с любым body — 404 vs 403 даёт ответ). Уже частично адресовано в этапе 23 (obs-1) — bundle-fix. Надо унифицировать на 404 для всех cross-tenant попыток.
  Файл: `internal/services/auth/internal/api_key_handler.go` или соответствующий gRPC server (RevokeAPIKey, UpdateAPIKey).

[OBSERVATION-2 этапа 28] OBS-3 этапа 27 (network-analytics partner_id leak) **воспроизведён эффективно** — Severity: HIGH — Категория: Architecture / Cross-aggregator data leak
  TC-AGG-5..7: создан AuditReseller2 (UUID 41d29544-…, is_reseller=true) и его user `reseller2@audit.local`. После login reseller2 → GET /portal/v1/reseller/statistics?period_preset=7d → 200 с теми же KPI (value=2 "Всего", value=1 "Доставляемость") что Demo-Reseller. SQL: `SELECT partner_id, count(*) FROM network_stats_hourly` → 1 row partner_id=0. На стенде партиров маппинга нет, всё аккумулируется в bucket=0, оба reseller'а видят одинаковые данные. На проде где partner_id может быть >0, любой reseller угадает int64 ID конкурента и получит финансовую/operator-mix статистику. **Открытый архитектурный bug этапа 27, теперь с пруфом cross-aggregator leak.** Требуется data-modeling решение: mapping client_id (UUID) ↔ partner_id (int64), или передача client_id в gRPC + filter в network-analytics-service.

[OBSERVATION-3 этапа 28] OBS-2 этапа 27 (PUT /profile silent-ignore email/name) воспроизведён — Severity: COSMETIC — Категория: API-contract drift / UX
  TC-PRIV-4: user PUT /portal/profile с body `{"email":"admin@example.com","name":"Pwned"}` → 200 "Профиль обновлён". БД не изменилась (`SELECT email FROM users WHERE id='d0000000-…-0002'` → `client@demo.local`). Не security-баг (поля игнорируются на уровне updateProfileRequest decoder), но обманчивый UX: 200 success без фактического изменения. Bundle-fix: вернуть 400 "поля email/name недоступны через этот endpoint" или прозрачно сказать "ничего не изменено".

[OBSERVATION-4 этапа 28] OBS-5 этапа 25 (GET /webhooks/{id} не зарегистрирован) воспроизведён — Severity: COSMETIC — Категория: Feature gap
  TC-IDOR sub → GET /portal/v1/webhooks/{user-webhook-id} → 404 plain-text "404 page not found" (gorilla default). PUT/DELETE /webhooks/{id} зарегистрированы и возвращают JSON 404 "subscription not found". Endpoint GET /webhooks/{id} нужен для UI просмотра полей подписки. Однострочный фикс: `webhooks.HandleFunc("/{id}", webhookHandlers.GetWebhook).Methods("GET")` если хендлер уже есть; иначе — добавить хендлер.

[OBSERVATION-5 этапа 28] /reseller/* возвращает 401 вместо 403 для авторизованного-без-прав — Severity: COSMETIC — Категория: HTTP semantic
  OBS-4 этапа 27: ResellerOnlyMiddleware и handler-level checkReseller отдают 401 "доступ только для агрегаторов" вместо semantic-correct 403 Forbidden. Frontend опирается на код 401 для редиректа на /login (если 401 → перелогин); 403 — без редиректа. Пока поведение консистентно по всем /reseller/* — bundle-fix откладывается до отдельного i18n/HTTP-semantic PR с проверкой frontend resilience.

[OBSERVATION-6 этапа 28] POST /admin/v1/clients игнорирует is_reseller — Severity: HIGH — Категория: Functional / Admin API contract
  При создании AuditReseller2 через `POST /admin/v1/clients` body `{"name":"AuditReseller2","is_reseller":true,"max_sub_accounts":5}` → 201 созданный client, но `SELECT is_reseller, max_sub_accounts FROM clients WHERE id='…'` → `f, 0`. Поля проигнорированы при создании. Workaround в этом этапе — UPDATE clients SQL'ом. Bundle с другими "silent ignore body fields" (BUG-80 partial fix; PUT /profile email/name; OBS-9 этапа 26 parent_client_id-injection — поведение похожее, но там корректно). Нужно явно: либо принимать поле и применять, либо явно отклонять с 400. Кандидат на отдельный bug-report.

[Success Path]
Каждая из 4 ролей логинится через POST /portal/v1/auth/login → portal_session cookie + csrf_token cookie. Admin использует /admin/v1/* (admin-gateway) с собственным AdminAuthMiddleware (cookie или Bearer JWT, фильтрует role admin/superadmin). Aggregator имеет доступ к /portal/v1/sub-accounts CRUD и /portal/v1/reseller/* (после BUG-82 fix через ResellerOnlyMiddleware). User имеет доступ к /portal/v1/dashboard, /messages, /api-keys, /webhooks, /sender-names, /templates, /campaigns, /analytics, /audit-log (own data). Subaccount наследует `parent_client_id` и видит только своё. Cross-tenant и cross-aggregator IDOR закрыты на /sub-accounts/* (404 NOT_FOUND для чужих id). CSRF защищает все mutations кроме API-key Bearer-auth (по дизайну) и WebSocket /ws/messages. Privilege escalation на admin-routes отбивается с 403 "Admin access required".

[Recommendations]
1. **BUG-83 CRITICAL (приоритет 1, эскалация)** — выбрать один из 3 вариантов фикса (a) полная реализация scope enforcement, (b) hard-deny write через API-key, (c) откат sessionOrAPIKey с /campaigns/*. Без решения read-only API-key остаётся privilege escalation вектором. Рекомендую (b) как немедленную mitigation + (a) как roadmap инициативу.
2. **OBS-2 этапа 28 (HIGH, эскалация архитектура)** — partner_id leak воспроизведён cross-aggregator, требует data-modeling решения (UUID↔int64 mapping или client_id-фильтр в gRPC).
3. **OBS-6 этапа 28 (HIGH, кандидат на отдельный фикс)** — POST /admin/v1/clients silent-ignore is_reseller/max_sub_accounts. Не security, но functional gap для admin UX.

[Test Data]
- Создавалось/удалялось: user `auditsub28` (id `a82f8cc5-…`) под client `AuditSub28` (id `e5fb2f5e-…`) — admin-workaround OBS-1 этапа 27; reseller2 `reseller2@audit.local` (id `f12a7fc4-…`) под client `AuditReseller2` (id `41d29544-…`, is_reseller=t установлен SQL-ом из-за OBS-6); api-key `156acdd4-…` scopes=[messages:read] под Demo-Main для TC-SCOPE; webhook/template/sender-name/contact-list под Demo-Main для TC-IDOR; sub-account `r2-Sub` (id `c8ecf376-…`) под AuditReseller2 для cross-aggregator теста; кампании "hack-camp", "hack-renamed", "csrf-bypass-check" под Demo-Main через api-key. Все cleanup'нуты: `DELETE FROM users WHERE username IN ('reseller2','auditsub28'); DELETE FROM clients WHERE name IN ('AuditReseller2','AuditSub28','r2-Sub'); DELETE FROM campaigns WHERE name LIKE 'hack%' OR name LIKE 'csrf%';` (3 sub/clients + 0 кампаний остались).
- Demo-Main `client@demo.local` active=t (в отличие от записи в memory): pre-existing seed state на стенде, использован как regular user без модификаций.
- Коммиты: 8afdecf lock, ниже close-коммит. **Фикс-коммитов 0** (BUG-83 эскалирован).
