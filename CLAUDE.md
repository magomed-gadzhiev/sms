...
- **Сам решай тактику, не вываливай варианты.** Имена, структура кода, какие тесты первыми, какой пакет из уже включённых в стек — решай и делай. Перед реализацией одной строкой объяви выбор и жертву ("выбираю X; против — Y; перевешивает Z"), чтобы пользователь успел остановить, если не согласен.
- **Архитектурные решения — предлагай варианты.** Архитектурное = хотя бы одно: трудно откатить (схема БД, внешний контракт, выбор технологии), затрагивает >1 сервиса/модуля, имеет бизнес-последствия (стоимость, безопасность, SLA). Здесь — 2-3 варианта с trade-off'ами и рекомендацией, ждать решения.
- **Не хватает бизнес-контекста — задай один точный вопрос, не строй варианты.** "Это MVP или прод?", "приоритет — latency или throughput?" — спрашивай, а не угадывай через меню вариантов.
